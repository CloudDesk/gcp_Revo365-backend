...
export class FinanceValidationError extends Error {
    constructor(message, statusCode = 400, code = "FINANCE_VALIDATION_ERROR") {
        super(message);
...
export const requireIsoDate = (value, fieldName) => {
...
export const toFinanceDateOnly = (value) => {
    if (value == null || value === "")
        return null;
...
export const normalizeEntrySide = (value) => {
    const normalized = String(value || "").trim().toLowerCase();
...
export const calculateAvailableBalance = (previousBalance, entrySide, amount) => {
...
export const protectAccountNumber = (accountNumber, secret) => {
    const normalized = String(accountNumber || "").replace(/\s+/g, "").trim();
    if (!/^[A-Za-z0-9-]{4,40}$/.test(normalized)) {
        throw new FinanceValidationError("accountnumber must contain 4 to 40 letters, numbers, or hyphens.");
    }
    const key = requireEncryptionKey(secret);
    const iv = Uint8Array.from(randomBytes(12));
    const cipher = createCipheriv("aes-256-gcm", key, iv);
    const encrypted = cipher.update(normalized, "utf8", "base64") + cipher.final("base64");
...
export const maskAccountNumber = (last4) => {
    const normalized = String(last4 || "").trim();
    return normalized ? `****${normalized}` : null;
};
export const formatTdsSectionDisplayName = (natureOfPayment, newCode, rate) => `${String(natureOfPayment).trim()} ${String(newCode).trim()}(${String(rate).trim()})`;
export const resolveFinanceContext = (request) => {
    const session = request?.session || {};
    const actor = session.useremail ??
        session.email ??
        session.id ??
        session.userid ??
        session.userId ??
        "unknown";
    const organizationId = Number(session.organizationid ?? session.organizationId ?? 1);
    return {
        actor: String(actor),
        organizationId: Number.isSafeInteger(organizationId) && organizationId > 0
            ? organizationId
            : 1,
    };
};
