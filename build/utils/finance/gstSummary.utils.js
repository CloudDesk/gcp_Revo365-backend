 ...
 */
export const parseGstMoney = (value) => {
...
export const resolveInvoiceGst = (invoice) => {
    ...
    const explicitSgst = firstAmount(invoiceData.sgstamount, summaryData.sgstamount, supportingData.sgstamount);
    const taxMode = normalizeTaxMode(invoiceData.taxmode, summaryData.taxmode, supportingData.taxmode, invoice?.taxmode);
    const taxType = normalizeTaxMode(invoiceData.taxtype, summaryData.taxtype, supportingData.taxtype);
    ...
    const legacyIgst = invoiceFor === "service" ? 0 : firstAmount(invoiceData.igst, summaryData.igst);
    const legacyCgst = invoiceFor === "service" ? 0 : firstAmount(invoiceData.cgst, summaryData.cgst);
    const legacySgst = invoiceFor === "service" ? 0 : firstAmount(invoiceData.sgst, summaryData.sgst);
    const serviceIgstRate = invoiceFor === "service"
        ? firstAmount(invoiceData.igst, summaryData.igst)
        : 0;
    const serviceCgstRate = invoiceFor === "service"
        ? firstAmount(invoiceData.cgst, summaryData.cgst)
        : 0;
    const serviceSgstRate = invoiceFor === "service"
...
export const resolveBillGst = (bill) => {
...
const sumGst = (rows, resolver) => {
    const result = (Array.isArray(rows) ? rows : []).reduce((sum, row) => {
...
export const getInvoiceGstSummary = (invoices) => sumGst(invoices, resolveInvoiceGst);
export const getBillGstSummary = (bills) => sumGst(bills, resolveBillGst);
//# sourceMappingURL=gstSummary.utils.js.map