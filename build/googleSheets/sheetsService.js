...
export const appendToSheet = async (sheetName, rowData) => {
    if (!WEBHOOK_URL) {
        console.warn("[sheetsService] ENQUIRY_SHEETS_WEBHOOK_URL is not set — skipping sheet write.");
        return;
    }
    const payload = {
        secret: SHEET_SECRET,
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (data?.success === false) {
                ...
                console.error(`[sheetsService] Apps Script error on sheet "${sheetName}":`, data.error);
                // Do NOT retry on logical errors — retrying won't help
                return;
            }
            console.log(`[sheetsService] ✅ Row appended to sheet "${sheetName}" | Row: ${data?.rowCount ?? "?"}`);
            return; // ✅ Success — exit
        }
        catch (err) {
            lastError = err;
            ...
            console.warn(`[sheetsService] Attempt ${attempt}/${MAX_RETRIES} failed for sheet "${sheetName}". Retrying in ${delay}ms...`);
            await sleep(delay);
        }
    }
    // All retries exhausted — log and move on (do NOT throw)
    console.error(`[sheetsService] ❌ Failed to append row to sheet "${sheetName}" after ${MAX_RETRIES} attempts. Error:`, lastError instanceof Error ? lastError.message : lastError);
};
