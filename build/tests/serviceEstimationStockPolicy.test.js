...
import { canRestoreApprovedServiceStock, getServiceEstimationStockPersistencePlan, } from "../services/serviceEstimationStockPolicy.js";
import { getServiceEstimationAssetAllocationPlan, isSingleAssetServiceEstimationUnit, isServiceEstimationCatalogueStock, resolveServiceEstimationAssetNumber, SERVICE_ESTIMATION_CATALOGUE_STOCK_TYPES, } from "../services/serviceEstimationStockEligibility.js";
import { getServiceEstimationTaxContext, resolveServiceEstimationCustomerState, } from "../services/serviceEstimationTaxPolicy.js";
const applyQuantityChange = (snapshot, action, quantity) => {
...
const plan = (isNew, previousStatus, nextStatus, allocations) => getServiceEstimationStockPersistencePlan({
    isNew,
    previousStatus,
    nextStatus,
    allocations,
...
describe("service estimation stock positive cases", () => {
    test("creating quantity 2 decreases Available from 5 to 3 and puts 2 on Service Hold", () => {
        ...
        const result = applyQuantityChange({ available: 5, serviceHold: 0, sold: 0 }, "hold", 2);
    ...
    test("approval keeps Available at 3 and moves the exact held quantity to Sold", () => {
        ...
        const result = applyQuantityChange({ available: 3, serviceHold: 2, sold: 0 }, "sell", 2);
        assert.deepEqual(result, {
            available: 3,
            serviceHold: 0,
            sold: 2,
    ...
    test("Walk-In auto-approval holds and sells in one plan", () => {
        ...
        assert.equal(decision.sellHeld, true);
        const held = applyQuantityChange({ available: 5, serviceHold: 0, sold: 0 }, "hold", 2);
    ...
    test("rejection and re-quote restore held stock to Available", () => {
        for (const status of ["rejected", "re_quote"]) {
            const decision = plan(false, "waiting_for_approval", status, { total: 2, sold: 0, restored: 0 });
            assert.equal(decision.restoreHeld, true);
            assert.equal(decision.heldRestorationReason, status === "rejected"
                ? "estimation_rejected"
                : "estimation_requote");
            const result = applyQuantityChange({ available: 3, serviceHold: 2, sold: 0 }, "restore-held", 2);
            assert.deepEqual(result, {
                available: 5,
                serviceHold: 0,
                sold: 0,
    ...
    test("after-approval cancellation restores Sold quantity to Available", () => {
        assert.equal(canRestoreApprovedServiceStock({
            estimationStatus: "approved",
            ticketStatus: "service_in_progress",
            invoiceGenerated: false,
            restorableQuantity: 2,
        }), true);
        const result = applyQuantityChange({ available: 3, serviceHold: 0, sold: 2 }, "restore-sold", 2);
        assert.deepEqual(result, {
            available: 5,
            serviceHold: 0,
            sold: 0,
...
describe("service estimation stock negative cases", () => {
    test("cannot reject or re-quote an approved estimation with sold stock", () => {
        for (const status of ["rejected", "re_quote"]) {
            assert.throws(() => plan(false, "approved", status, {
                total: 2,
                sold: 2,
                restored: 0,
    ...
    test("cannot approve an estimation after its tracked stock was restored", () => {
        assert.throws(() => plan(false, "approved", "approved", {
            total: 2,
            sold: 0,
            restored: 2,
    ...
    test("quantity cannot exceed Available and cannot be restored twice", () => {
        assert.throws(() => applyQuantityChange({ available: 1, serviceHold: 0, sold: 0 }, "hold", 2));
        assert.throws(() => applyQuantityChange({ available: 5, serviceHold: 0, sold: 0 }, "restore-sold", 1));
    });
});
describe("service estimation catalogue stock eligibility", () => {
    test("Available On Catalogue and Off Catalogue stock are eligible", () => {
        ...
        for (const stocktype of SERVICE_ESTIMATION_CATALOGUE_STOCK_TYPES) {
            assert.equal(isServiceEstimationCatalogueStock({
                stocktype,
    ...
    test("selected asset is reserved first and remaining quantity stays automatic", () => {
        assert.deepEqual(getServiceEstimationAssetAllocationPlan(3, [101]), {
            selectedStockIds: [101],
            remainingQuantity: 2,
        });
        assert.deepEqual(getServiceEstimationAssetAllocationPlan(2, [101, 102]), {
            selectedStockIds: [101, 102],
            remainingQuantity: 0,
        });
    });
    test("asset display matches In-Store Sales RFID-first mapping", () => {
        assert.equal(resolveServiceEstimationAssetNumber({
            rfid: "2341234123",
    ...
    test("duplicate, invalid, or excessive selected assets are rejected", () => {
        assert.throws(() => getServiceEstimationAssetAllocationPlan(2, [101, 101]), /same asset number/);
        assert.throws(() => getServiceEstimationAssetAllocationPlan(1, [101, 102]), /cannot exceed product quantity/);
        assert.throws(() => getServiceEstimationAssetAllocationPlan(1, [0]), /stock id is invalid/);
    });
});
describe("service estimation customer-state tax policy", () => {
    test("ticket address has priority and customer address is the fallback", () => {
        assert.equal(resolveServiceEstimationCustomerState("Tamil Nadu", "Assam"), "Tamil Nadu");
        assert.equal(resolveServiceEstimationCustomerState(null, "Assam"), "Assam");
        assert.equal(resolveServiceEstimationCustomerState("", ""), "Tamil Nadu");
    });
...
//# sourceMappingURL=serviceEstimationStockPolicy.test.js.map