...
import { applyRetailInvoiceAllocation, getRetailInvoicePaymentState, isRetailStoreInvoice, isServiceRequestInvoice, resolveCustomerReceiptSourceType, } from "../utils/finance/retailReceipt.utils.js";
...
describe("Service Request invoice receipt flow", () => {
    test("normal and walk-in tickets use the same isolated service eligibility", () => {
        assert.equal(isServiceRequestInvoice({
    ...
    test("partial Bank receipt plus TDS settles only the combined invoice amount", () => {
        const allocation = applyRetailInvoiceAllocation(generatedServiceInvoice, 4000, 400);
    ...
    test("Bank/Cash balance increases by receipt amount and excludes TDS", () => {
        ...
        assert.equal(calculateAvailableBalance(openingBankBalance, "debit", receiptAmount), 24000);
    ...
    test("a later receipt can complete a partially paid service invoice", () => {
        ...
        const finalAllocation = applyRetailInvoiceAllocation(partiallyPaidInvoice, 5000, 600);
    ...
    test("overpayment and allocation plus excessive TDS are rejected", () => {
        assert.throws(() => applyRetailInvoiceAllocation(generatedServiceInvoice, 10001), /exceeds its outstanding amount/);
        assert.throws(() => applyRetailInvoiceAllocation(generatedServiceInvoice, 9500, 501), /exceeds its outstanding amount/);
    });
    ...
    test("Service Request receipt uses its own source classification", () => {
        assert.equal(resolveCustomerReceiptSourceType([generatedServiceInvoice]), "service_request_receipt");
    });
    test("a mixed-source payment is classified as a customer receipt", () => {
        ...
        assert.equal(resolveCustomerReceiptSourceType([generatedServiceInvoice, storeInvoice]), "customer_receipt");
    });
});
describe("Existing invoice flow regression boundaries", () => {
    test("in-store product invoice remains retail and not service", () => {
        ...
        assert.equal(resolveCustomerReceiptSourceType([storeInvoice]), "retail_receipt");
    });
    test("online product order remains eligible only for e-commerce automation", () => {
        ...
        assert.equal(isRetailStoreInvoice({
            invoicefor: "product",
...
//# sourceMappingURL=serviceRequestInvoiceFlow.test.js.map