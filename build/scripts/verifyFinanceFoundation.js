...
const verify = async () => {
    ...
    if (versionResult.rows.length !== 7) {
        failures.push("Missing the frozen Cash and Bank Phase 1 release, Retail Receipt migration, or one of their schema versions.");
    }
    ...
    if (systemAccountResult.rows.length !== 7) {
        failures.push(`Expected 7 active system accounts, found ${systemAccountResult.rows.length}.`);
    }
    ...
    if (tdsSectionResult.rows.length !== 12) {
        failures.push(`Expected 12 seeded TDS sections, found ${tdsSectionResult.rows.length}.`);
    }
    ...
    `);
    const financePermissionsAreValid = financePermissionResult.rows.length === 2 &&
        financePermissionResult.rows.every((row) => row.permissions?.read === true &&
            row.permissions?.create === true &&
            row.permissions?.edit === true &&
            row.permissions?.delete === false);
    if (!financePermissionsAreValid) {
        failures.push("Admin and Accountant must have standard read/create/edit Cash and Bank permissions.");
    }
    ...
    `);
    const accountNameIndexes = new Map(accountNameIndexResult.rows.map((row) => [row.indexname, row.indexdef]));
    const duplicateNamesAreAllowed = !accountNameIndexes.has("uq_bank_cash_accounts_name") &&
    ...
    if (!duplicateNamesAreAllowed) {
        failures.push("Cash/Bank and finance account names must use non-unique search indexes.");
    }
    ...
    if (!ecommerceDefaultIndexResult.rows[0]) {
        failures.push("Cash/Bank accounts must enforce one e-commerce default per organization.");
    }
    ...
    LIMIT 1
    `);
    const ecommerceDefaultConstraint = String(ecommerceDefaultConstraintResult.rows[0]?.definition || "").toLowerCase();
    if (!ecommerceDefaultConstraint.includes("accounttype") ||
        !ecommerceDefaultConstraint.includes("'bank'")) {
        failures.push("The e-commerce default must be restricted to an active Bank account.");
    }
    ...
    if (!retailAllocationIndexResult.rows[0]) {
        failures.push("Retail invoice allocations must enforce one active allocation per transaction and invoice.");
    }
    ...
    `);
    if (ecommerceIdempotencyResult.rows[0]?.eventunique !== true ||
        ecommerceIdempotencyResult.rows[0]?.transactionunique !== true) {
        failures.push("E-commerce payments must have event and Bank transaction idempotency controls.");
    }
    if (failures.length > 0) {
        throw new Error(`[Finance Foundation Verification]\n${failures
            .map((failure) => `- ${failure}`)
            .join("\n")}`);
    }
    console.log(`[Finance Foundation Verification] Passed: frozen Phase 1 release, Retail Receipt controls, ${Object.keys(expectedColumns).length} tables, 7 system accounts, 12 TDS sections, and Admin/Accountant permissions.`);
};
verify().catch((error) => {
    console.error(error?.message || error);
    process.exitCode = 1;
});
//# sourceMappingURL=verifyFinanceFoundation.js.map