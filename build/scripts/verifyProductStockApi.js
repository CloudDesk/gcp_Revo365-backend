...
import { connectGetSessionredis, redisClient, } from "../database/redis.session.js";
...
const expectSuccess = (result, context) => {
    assert.equal(result.status, 200, `${context} returned ${result.status}: ${JSON.stringify(result.body)}`);
    return result.body;
};
const getActiveValues = async (definitionCode) => {
    const result = await query(`SELECT value.id, value.code, value.label, value.value, value.metadata_json
     ...
     ORDER BY value.sort_order, value.label`, [definitionCode]);
    return result.rows;
};
const findCreatedProduct = async (productname) => {
    const result = await query(`SELECT id, puc, productname, category, subcategory, producttype, buildtype,
...
const createProduct = async (payload, context) => {
    const response = expectSuccess(await request("POST", "/v2/product", payload), context);
    responseContracts.productHasMessage && (responseContracts.productHasMessage = typeof response?.message === "string");
    responseContracts.productHasRecord && (responseContracts.productHasRecord = Boolean(response?.record?.id) || Boolean(response?.data?.id));
    const product = await findCreatedProduct(payload.productname);
    const readBody = expectSuccess(await request("GET", `/v2/product/${product.id}`), `${context} read-back`);
...
const createStock = async (product, index, stockType) => {
    const taxField = stockType === "rental_product" ? "saccode" : "hsncode";
    const taxValue = stockType === "rental_product" ? "997315" : "84713010";
    const taxResponse = expectSuccess(await request("POST", "/v2/product", {
        id: product.id,
        [taxField]: taxValue,
    }), `Product tax update ${product.productname}`);
    responseContracts.productHasMessage && (responseContracts.productHasMessage = typeof taxResponse?.message === "string");
    ...
    const response = expectSuccess(await request("POST", "/v2/stock", payload), `Stock create ${product.productname}`);
    responseContracts.stockHasMessage && (responseContracts.stockHasMessage = typeof response?.message === "string");
    responseContracts.stockHasRecord && (responseContracts.stockHasRecord = Boolean(response?.stock?.id) || Boolean(response?.data?.id));
    const persisted = await query(`SELECT id, puc, productname, serialnumber, stocktype, stockstatus, rfid
     FROM stock_revo
     WHERE serialnumber = $1`, [serialnumber]);
    assert.equal(persisted.rows.length, 1, `Stock was not persisted for ${product.productname}`);
    ...
    const readBody = expectSuccess(await request("GET", `/v2/stock/${stock.id}`), `Stock read-back ${product.productname}`);
    ...
    assert.equal(readBody[0]?.[taxField], taxValue, `Stock GET did not expose Product ${taxField}`);
    const quantityResult = await query(`SELECT quantity, oncatalogueqty, offcatalogueqty, rentaltotalquantity,
            rentalavailablequantity
     FROM product_revo
     WHERE id = $1`, [product.id]);
    ...
    const listBody = expectSuccess(await request("GET", `/v2/stock?serialnumber=${encodeURIComponent(serialnumber)}&count=10`), `Stock filtered list ${product.productname}`);
...
const cleanup = async () => {
    ...
    try {
        ...
        await query(`DELETE FROM product_bom_lines
       WHERE product_bom_id IN (
        ...
        await query(`DELETE FROM product_boms
       WHERE product_id IN (
...
const main = async () => {
    ...
    const stockTypesResult = await query(`SELECT DISTINCT stocktype
     FROM stock_revo
     WHERE stocktype IS NOT NULL
     ORDER BY stocktype`);
    const stockTypes = stockTypesResult.rows.map((row) => row.stocktype);
    const supplierResult = await query(`SELECT id FROM supplier
     WHERE (isdeleted = FALSE OR isdeleted IS NULL)
     ORDER BY id
     LIMIT 1`);
    const supplierId = supplierResult.rows[0]?.id;
    assert.deepEqual(categories.map((value) => value.code), ["new", "refurbished"], "Unexpected active Product conditions");
    ...
    const spareProductsByRole = new Map();
    let caseIndex = 0;
    for (const category of categories) {
        for (const role of roles) {
            ...
            if (category.code === "new")
                spareProductsByRole.set(role.code, product);
            await createStock(product, caseIndex, stockTypes[(caseIndex - 1) % stockTypes.length]);
            const options = expectSuccess(await request("GET", `/v2/product/component-options?component_role=${encodeURIComponent(role.code)}&search=${encodeURIComponent(product.productname)}`), `Component lookup ${role.code}`);
            ...
            assert.equal(options[0]?.role_match_rank, 0, `Component lookup did not rank ${role.code} as an exact match`);
        }
    }
    ...
    for (const category of categories) {
        for (const packageType of packageTypes) {
            for (const [buildType, fulfillmentType] of buildFulfilments) {
                caseIndex += 1;
                const requiredRoles = roles.filter((role) => (role.metadata_json?.requiredWhen || []).some((rule) => (!rule.packageType || rule.packageType === packageType) &&
                ...
                const bomcomponents = requiredRoles.map((role, index) => {
                    ...
                    return {
                        ...
                        quantity: role.code === "monitor" && packageType === "computer_full_set"
                            ? 2
                            : 1,
                        is_required: true,
                ...
                await createStock(product, caseIndex, stockTypes[(caseIndex - 1) % stockTypes.length]);
                const bom = expectSuccess(await request("GET", `/v2/product/${product.id}/bom`), `BOM read-back ${product.productname}`);
                ...
                assert.equal(bom?.fulfillment_type, fulfillmentType, "BOM fulfillment type mismatch");
                assert.equal(bom?.components?.length, requiredRoles.length, "BOM component count mismatch");
    ...
    const remainingProducts = await query(`SELECT COUNT(*)::int AS count
     FROM product_revo
     WHERE productname LIKE $1`, [`${productPrefix}%`]);
    const remainingStocks = await query(`SELECT COUNT(*)::int AS count
     FROM stock_revo
     WHERE serialnumber LIKE $1`, [`${serialPrefix}%`]);
    assert.equal(remainingProducts.rows[0].count, 50, "Expected 50 Product matrix records before cleanup");
    assert.equal(remainingStocks.rows[0].count, 50, "Expected 50 Stock matrix records before cleanup");
    return {
        runId,
        products: createdProductIds.length,
        stocks: createdStockIds.length,
        spareCases: categories.length * roles.length,
        computerCases: categories.length * packageTypes.length * buildFulfilments.length,
        stockTypes,
        responseContracts,
        failures,
    };
...
//# sourceMappingURL=verifyProductStockApi.js.map