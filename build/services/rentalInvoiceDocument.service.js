...
const sumQuantities = (items) => items.reduce((total, item) => total + Math.max(toNumber(item?.quantity), 0), 0);
...
const getDefaultLogoDataUrl = async () => {
    ...
    try {
        ...
        console.warn(`Rental summary invoice logo not found at ${DEFAULT_LOGO_PATH}: ${error?.message || error}`);
    }
    return cachedDefaultLogoDataUrl;
};
...
const fetchRentalOrderLines = async (uniqueOrderId) => {
    if (!uniqueOrderId) {
        return [];
    }
...
const buildSummaryData = (invoice, orderLines, options) => {
    ...
    const invoiceCreatedDate = parseDateValue(invoice?.createddate) || parseDateValue(Date.now());
    ...
    const sacCode = firstPresentText(firstInvoiceItem?.saccode, firstInvoiceItem?.hsncode, firstOrderLine?.saccode, firstOrderLine?.hsncode, "997315");
    ...
    const totalAmount = toNumber(invoiceData?.total) ||
...
(function (rentalInvoiceDocumentService) {
    rentalInvoiceDocumentService.generateRentalInvoiceDocuments = async (invoiceId, options = {}) => {
        ...
        const { summaryInvoiceData, supportingDocumentData, templateData } = buildSummaryData(invoice, orderLines, { ...options, logoUrl });
