...
import { assertTransactionDateIsWithinAccountHistory, rebuildBankCashAccountBalances, } from "../utils/finance/bankTransactionBalance.utils.js";
...
const selectReceivableInvoices = (mode) => `
  SELECT
    r.*,
...
const serializeOutstandingInvoice = (row) => {
    ...
    return {
        ...
        paymentstatus: state.outstandingAmount === 0
            ? "paid"
            : state.paidAmount > 0
                ? "partially_paid"
                : "pending",
...
export var retailReceiptFinanceService;
(function (retailReceiptFinanceService) {
    retailReceiptFinanceService.listCustomers = async (request) => {
        ...
        for (const row of invoiceRows) {
            if (!isEligibleCustomerReceiptInvoice(row, receiptMode))
                continue;
            const state = getRetailInvoicePaymentState(row);
            if (state.outstandingAmount <= 0)
                continue;
            ...
            customer.totaloutstanding = toMoney(customer.totaloutstanding + state.outstandingAmount);
    ...
    retailReceiptFinanceService.listOutstandingInvoices = async (request) => {
        const customerId = Number(request.params?.customerId);
        const receiptMode = resolveCustomerReceiptMode(request.query?.mode);
        if (!Number.isSafeInteger(customerId) || customerId <= 0) {
            throw new FinanceValidationError("A valid customerId is required.");
        }
        const result = await query(`${selectReceivableInvoices(receiptMode)}
       ORDER BY COALESCE(r.invoicedate, r.createddate) ASC, r.id ASC`, [customerId]);
        return result.rows
            .filter((invoice) => isEligibleCustomerReceiptInvoice(invoice, receiptMode))
            .map(serializeOutstandingInvoice)
            .filter((invoice) => invoice.outstandingamount > 0);
    };
    retailReceiptFinanceService.postReceipt = async (request) => {
        ...
        if (requestedAllocations.length === 0) {
            throw new FinanceValidationError("At least one invoice allocation is required.");
        }
        const normalizedAllocations = requestedAllocations.map((item) => {
            ...
            if (tdsApplied) {
                if (tdsAmount <= 0) {
                    throw new FinanceValidationError("TDS Receivable amount must be greater than zero when TDS is applied.");
                }
            }
            else if (tdsAmount !== 0) {
                throw new FinanceValidationError("TDS Receivable amount must be zero when TDS is not applied.");
            }
            return {
                invoiceId,
                allocationAmount,
        ...
        if (invoiceIds.some((id) => !Number.isSafeInteger(id) || id <= 0) ||
            new Set(invoiceIds).size !== invoiceIds.length) {
            throw new FinanceValidationError("Invoice allocations must contain unique valid invoice IDs.");
        }
        ...
        if (allocationTotal !== amount) {
            throw new FinanceValidationError("Receipt amount must equal the total invoice allocation.");
        }
        const client = await pool.connect();
        try {
            await client.query("BEGIN");
            const existingReceipt = await getExistingReceipt(client, organizationId, requestReference);
            if (existingReceipt) {
        ...
        `, [accountId, organizationId]);
            ...
            if (account.status !== "active") {
                throw new FinanceValidationError("Receipts can only be posted to an active Bank/Cash account.");
            }
        ...
        `, [invoiceIds]);
            if (invoiceResult.rows.length !== invoiceIds.length) {
                throw new FinanceValidationError("One or more selected invoices were not found.");
            }
            const invoiceById = new Map(invoiceResult.rows.map((row) => [Number(row.id), row]));
            const preparedAllocations = normalizedAllocations.map((item) => {
                ...
                if (!invoice ||
                    ...
                    throw new FinanceValidationError("All selected invoices must be eligible outstanding invoices for the selected customer.");
                }
                return {
                    invoice,
                    tdsApplied: item.tdsApplied,
                    ...applyRetailInvoiceAllocation(invoice, item.allocationAmount, item.tdsAmount),
                };
            });
            assertTransactionDateIsWithinAccountHistory(transactionDate, account.openingbalancedate);
        ...
        `, [organizationId]);
            ...
            if (!Number.isSafeInteger(accountsReceivableId)) {
                throw new FinanceValidationError("Accounts Receivable system ledger is unavailable.", 409, "ACCOUNTS_RECEIVABLE_LEDGER_MISSING");
            }
            let tdsReceivableAccountId = null;
            if (totalTdsAmount > 0) {
          ...
          `, [organizationId]);
                ...
                if (!Number.isSafeInteger(tdsReceivableAccountId)) {
                    throw new FinanceValidationError("TDS Receivable system ledger is unavailable.", 409, "TDS_RECEIVABLE_LEDGER_MISSING");
                }
            }
        ...
        `, [transactionNumber, journalEntry.id, bankTransaction.id]);
            const rebuiltBalances = await rebuildBankCashAccountBalances(client, account, actor, epoch);
            const transactionBalanceAfter = rebuiltBalances.balances.get(Number(bankTransaction.id)) ?? balanceAfter;
...
})(retailReceiptFinanceService || (retailReceiptFinanceService = {}));
//# sourceMappingURL=retailReceiptFinance.service.js.map