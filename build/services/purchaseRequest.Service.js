    ...
    purchaseRequestService.upsertPurchaseRequestData = async (prData) => {
        ...
        try {
            ...
            if (prData.isDemandRequest) {
                ...
                const demandRequestRow = await query('SELECT demandrequestdata FROM demandrequest WHERE id = $1', [prData.demandRequestId]);
                if (!demandRequestRow.rows.length) {
                ...
                for (const inputProduct of prdataInput) {
                    ...
                    const existingProduct = demandrequestdata.find((item) => item.name === inputProduct.name);
                    if (existingProduct && Array.isArray(existingProduct.prdata)) {
                        // Check if there is any PR with status 'Completed'
                        const hasCompleted = existingProduct.prdata.some((pr) => pr.prstatus === 'closed_won');
                        if (hasCompleted) {
                            throw new Error(`Cannot add/update PR, PR is already completed.`);
                        }
                    }
                }
            }
