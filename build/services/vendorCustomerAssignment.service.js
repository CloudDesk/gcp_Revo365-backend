...
const normalizeRole = (value) => String(value ?? "")
    .trim()
    .toLowerCase();
const parsePositiveIntegerArray = (values) => {
...
const requireAdmin = async (request) => {
    ...
    if (normalizeRole(session?.role) !== "admin") {
        const error = new Error("Only admin users can manage vendor customer assignments.");
        error.statusCode = 403;
        throw error;
    }
    return session;
};
export var vendorCustomerAssignmentService;
(function (vendorCustomerAssignmentService) {
    vendorCustomerAssignmentService.getAssignments = async (request) => {
        const session = await accessScopeService.getSession(request);
        ...
        if (!isAdmin && (!isVendor || Number(session?.id) !== requestedVendorUserId)) {
            const error = new Error("You are not allowed to view these assignments.");
            error.statusCode = 403;
            throw error;
        }
    ...
    vendorCustomerAssignmentService.replaceAssignments = async (request) => {
        ...
        if (!vendorUserId) {
            const error = new Error("Vendor user id is required.");
            error.statusCode = 400;
            throw error;
        }
        const vendorResult = await query(`SELECT id, role FROM inventoryusers WHERE id = $1 LIMIT 1`, [vendorUserId]);
        const vendor = vendorResult.rows[0];
        if (!vendor || normalizeRole(vendor.role) !== "vendor") {
            const error = new Error("Selected inventory user must have the Vendor role.");
            error.statusCode = 400;
            throw error;
        }
        ...
        `, [customerIds]);
            const validCustomerIds = new Set(businessCustomersResult.rows.map((row) => Number(row.id)));
            const invalidCustomerIds = customerIds.filter((id) => !validCustomerIds.has(id));
            if (invalidCustomerIds.length > 0) {
                const error = new Error("Assignments can include only business customers.");
                error.statusCode = 400;
...
})(vendorCustomerAssignmentService || (vendorCustomerAssignmentService = {}));
//# sourceMappingURL=vendorCustomerAssignment.service.js.map