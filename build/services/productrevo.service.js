...
(function (productrevoService) {
  ...
  productrevoService.getproductsData = async (request) => {
    ...
    try {
      ...
      if (error.message.startsWith("Timeout occurred during")) {
        ...
        console.error(
          `[${new Date().toISOString()}] [${requestId}] Error in getproductsData after ${duration} ms:`,
          error
        );
      }
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
  ...
  productrevoService.getSimilarProducts = async (request) => {
    try {
      const pageNumber = parseInt(request.query.page) || 1;
      const recordCount = parseInt(request.query.count) || 5000;
      const keys = Object.keys(request.query);
      ...
      let queryText = `SELECT * FROM product_revo ${whereClause} ${orderByClause} OFFSET $${parameterIndex} LIMIT $${
        parameterIndex + 1
      }`;
      queryParams.push(offset, recordCount);
      console.log("Query Text:", queryText);
      ...
      if (result.rows.length <= 1) {
        ...
        keys.forEach((key, index) => {
          if (key === "subcategory") {
            ...
            const whereClauseLatest = `(${clauses.join(
              " OR "
            )}) AND ${baseConditions}`;
  ...
  productrevoService.deleteProductrevo = async (id) => {
    try {
      const result = await query(`DELETE FROM product_revo WHERE id = $1`, [
        id,
      ]);
  ...
  productrevoService.getArcheivedProductsrevo = async (request) => {
    try {
      ...
      let queryText = `SELECT * FROM product_revo`;
      if (whereClause) {
        queryText += ` WHERE ${whereClause} AND isarchive = true AND removefromrecyclebin = false  OFFSET $${parameterIndex} LIMIT $${
          parameterIndex + 1
        }`;
      } else if (pageNumber && recordCount) {
        queryText += ` WHERE isarchive = true AND removefromrecyclebin = false  OFFSET $${parameterIndex} LIMIT $${
          parameterIndex + 1
        }`;
        queryParams.push(offset, recordCount);
      } else {
      ...
      console.error(
        "Query Execution Error: IN getArcheivedProductsrevo",
        error
      );
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
      console.log(ErrorMessage);
  ...
  productrevoService.upsertProductwithFileRevo = async (request) => {
    try {
      ...
      console.error(
        "Query Execution Error: IN upsertProductwithFileRevo",
        error
      );
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
      console.log(ErrorMessage);
  ...
  productrevoService.updateAvgRatingProductrevo = async (
    avgRating,
    productid
  ) => {
    console.log(
      "inside Update avg rating in productrevo",
      avgRating,
      productid
    );
    try {
      const result = await query(
        `UPDATE product_revo SET averagerating = $1 WHERE id = $2`,
        [avgRating, productid]
      );
      ...
      console.error(
        "Query Execution Error: IN updateAvgRatingProductrevo",
        error
      );
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
      console.log(ErrorMessage);
  ...
  productrevoService.upsertQuantityFields = async (
    upsertData,
    orderedquantitydata,
    issold
  ) => {
    ...
    try {
      ...
      let productquery = await query(
        `SELECT orderedquantity FROM product_revo WHERE puc = $1`,
        [puc]
      );
      ...
      console.log(
        orderedquantitydata,
        "orderedquantityNumber bEFOQRE conversion"
      );
      let orderedquantityNumber = Number(orderedquantitydata);
      console.log(
        orderedquantitydata,
        "orderedquantityNumber after conversion"
      );
      ...
      if (
        updateCartQuantity?.command === "UPDATE" ||
  ...
  productrevoService.testupsertQuantityFieldsBatch = async (
    batchData,
    issold
  ) => {
    try {
      ...
      const updatePromises = updateQueries.map((update) =>
  ...
  productrevoService.updateCancelledOrderedQuantity =
    updateCancelledOrderedQuantity;
