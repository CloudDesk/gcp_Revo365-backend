...
(function (productrevoService) {
     ...
     *   }
     */
    const buildStatusHistoryPayload = (previousHistory, resolvedActor, ecomVisible) => {
        const newEntry = {
            active: true,
        ...
        const existing = Array.isArray(previousHistory)
            ? previousHistory.map((e) => ({ ...e, active: false }))
            : [];
    ...
    productrevoService.getproductsData = async (request, visibilityMode) => {
        try {
    ...
    productrevoService.getEcomProducts = async (request, visibilityMode = "visible") => {
        try {
            const pageNumber = parseInt(request.query.page) || 1;
            const recordCount = parseInt(request.query.count) || 5000;
            const keys = Object.keys(request.query);
            ...
            keys.forEach((key, index) => {
                let paramValues = Array.isArray(values[index]) ? values[index] : [values[index]];
                if (key === "displaysize" || key === "price") {
                    const rangeClauses = paramValues.map(range => {
                        const [lowerBound, upperBound] = range.split("-");
                        queryParams.push(lowerBound, upperBound);
    ...
    productrevoService.getSimilarProducts = async (request, visibilityMode = "visible") => {
        try {
            const pageNumber = parseInt(request.query.page) || 1;
            const recordCount = parseInt(request.query.count) || 5000;
            const keys = Object.keys(request.query);
     ...
     */
    productrevoService.toggleEcomVisible = async (id, ecomVisible, actor) => {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            // 1. Fetch & lock the product row
            const productResult = await client.query(`SELECT id, puc, ecomvisible, statushistory FROM product_revo WHERE id = $1 FOR UPDATE`, [id]);
            ...
            if (actor?.useremail) {
                const userLookup = await client.query(`SELECT id, firstname, lastname, role
           FROM inventoryusers
           WHERE useremail = $1
           LIMIT 1`, [actor.useremail]);
         ...
         SET ecomvisible    = $1,
             statushistory  = $2
         WHERE id = $3`, [ecomVisible, JSON.stringify(newStatusHistory), id]);
            // 5. If Toggle OFF: clear cart/wishlist
            let cartClearedCount = 0;
            if (!ecomVisible) {
                const deleteResult = await client.query(`DELETE FROM cart WHERE productid = $1 RETURNING id`, [id]);
    ...
    //get
    productrevoService.getEachProductsRevo = async function (request, id, visibilityMode) {
        try {
            const visibilityClause = visibilityMode ? ` AND ${getVisibilityCondition(visibilityMode)}` : '';
    ...
    productrevoService.upsertProductwithFileRevo = async (request) => {
        try {
            const { productid } = request.params;
            let existingProductData = {};
            const upsertProductData = [];
            if (productid) {
                existingProductData = await query(`SELECT * FROM product_revo where id=${productid}`, {});
            }
            let data = {};
    ...
    productrevoService.upsertProductwithfileRevogcp = async (request) => {
        try {
    ...
    productrevoService.rearrangeImageRevo = async (request) => {
        try {
            const { productid } = request.params;
            ...
            let querydata;
            let params = [];
            let getData = await query(`select large,medium,small from product_revo where id =${productid}`, {});
    ...
    productrevoService.updateRemoveFromRecyclebinRevo = async () => {
        const updateQuery = `
            UPDATE product_revo
    ...
    productrevoService.updateAvgRatingProductrevo = async (avgRating, productid) => {
        try {
            const result = await query(`UPDATE product_revo SET averagerating = $1 WHERE id = $2`, [avgRating, productid]);
            if (result.rowCount != 0) {
    ...
    productrevoService.updateoverallAvailableQuantity = async (puc) => {
        try {
    ...
    `;
            ...
            if (issold && !isNaN(orderedquantityNumber)) {
                ...
                if (isRental) {
                    ...
                    updateQueryBase += `, rentalorderedquantity = GREATEST(0, COALESCE(rentalorderedquantity, 0) - $15)`;
          ...
          UPDATE product_revo 
          SET quantity = $1, 
              ecompublishedquantity = GREATEST(0, $2 - GREATEST(0, COALESCE(orderedquantity, 0) - $15)), 
              soldquantity = $3, 
              availablequantity = $4, 
              productstatus = $5, 
              overallavailableqty = GREATEST(0, $6 - GREATEST(0, COALESCE(orderedquantity, 0) - $15)), 
              rentalsoldquantity = $7,
              oncatalogueqty = $8,
              offcatalogueqty = $9,
              rentaltotalquantity = $10,
              ...
              archive_qty = $13,
              ewaste_qty = $14,
              orderedquantity = GREATEST(0, COALESCE(orderedquantity, 0) - $15)
          WHERE puc = $16 RETURNING *`;
                }
            }
            else {
    ...
    const syncGlobalProductOrderedQuantities = async (pucs) => {
    ...
    async function updateOrderedQuantity(productIds, orderedquantity) {
        try {
            return 'resultdata';
        }
        catch (error) {
    ...
    async function releaseCommittedQuantityForOrderLines(lines, restoreAvailability) {
        try {
            const normalLines = new Map();
            const rentalLines = new Map();
            for (const line of lines || []) {
                if (line?.ordertype && line.ordertype !== "Orders")
                    continue;
            ...
            for (const [productId, quantity] of normalLines) {
             ...
             WHERE id = $1
             RETURNING puc`, [productId, quantity])
                    : await query(`UPDATE product_revo
             SET orderedquantity = GREATEST(0, COALESCE(orderedquantity, 0) - $2)
             WHERE id = $1
             RETURNING puc`, [productId, quantity]);
    ...
    productrevoService.releaseCommittedQuantityForOrderLines = releaseCommittedQuantityForOrderLines;
})(productrevoService || (productrevoService = {}));
//# sourceMappingURL=productrevo.service.js.map