...
const CONSOLIDATED_INVOICE_FOLDER = process.env.CONSOLIDATED_INVOICE_FOLDER || "consolidated-invoices";
...
const roundToNearestRupeePaise = (value) => Math.round(toPaise(value) / 100) * 100;
const prorateAmount = (value, billableDays, cycleDays) => {
...
const formatProductSubcategoryLabel = (value) => {
...
const inferProductSubcategoryFromText = (value) => {
    const text = normalizeComparable(value).replace(/[_-]+/g, " ");
    if (!text)
        return "";
    if (/\blaptops?\b/.test(text))
        return "laptop";
    if (/\b(mobiles?|phones?|mobile phone)\b/.test(text))
        return "mobile_phone";
    if (/\b(accessories|accessory)\b/.test(text))
        return "accessories";
    if (/\bcomputers?\b/.test(text))
        return "computer";
    if (/\bspares?\b/.test(text))
        return "spares";
    return "";
};
const getDatePart = (date, part) => new Intl.DateTimeFormat("en-CA", {
...
const normalizeDate = (date) => dateKeyToUtcDate(formatDateKey(date));
...
const shiftDateByMonths = (baseDate, months) => {
    ...
    if (shiftedDate.getUTCDate() < originalDay)
        shiftedDate.setUTCDate(0);
    return shiftedDate;
};
const getDefaultBillingPeriodEnd = (billingPeriodStart) => addDays(shiftDateByMonths(billingPeriodStart, 1), -1);
const compareDateKeys = (left, right) => formatDateKey(left).localeCompare(formatDateKey(right));
const maxDate = (...dates) => dates.reduce((latest, date) => (compareDateKeys(date, latest) > 0 ? date : latest));
const minDate = (...dates) => dates.reduce((earliest, date) => (compareDateKeys(date, earliest) < 0 ? date : earliest));
const daysBetweenInclusive = (startDate, endDate) => {
    const start = normalizeDate(startDate);
    const end = normalizeDate(endDate);
    return Math.floor((end.getTime() - start.getTime()) / DAY_IN_MS) + 1;
};
const isAfterDate = (left, right) => compareDateKeys(left, right) > 0;
...
const formatPeriodLabel = (date) => new Intl.DateTimeFormat("en-GB", {
    timeZone: BILLING_TIMEZONE,
    month: "long",
    year: "numeric",
}).format(date);
...
const getPeriodFromInput = (requestBody) => {
    const rawPeriod = normalizeText(requestBody?.period) ||
...
const getBillingThroughDate = (requestBody) => {
    const rawBillingThroughDate = normalizeText(requestBody?.billingthroughdate) ||
...
const normalizeInvoiceForValues = (value) => {
...
const getInvoiceForKey = (invoiceForValues) => [...invoiceForValues].sort().join(",");
const isDateWithinPeriod = (date, startDate, endDate) => {
    if (!date)
        return false;
    const key = formatDateKey(date);
    return key >= formatDateKey(startDate) && key <= formatDateKey(endDate);
};
const getEffectiveInvoiceDate = (invoice, invoiceFor) => {
    if (invoiceFor === "rental") {
        return (parseDateValue(invoice?.billingperiodstart) ||
...
const getRentalBillingPeriod = (invoice) => {
    const summaryInvoiceData = parseJsonValue(invoice?.summaryinvoicedata, {});
    const startDate = parseDateValue(invoice?.billingperiodstart) ||
        parseDateValue(summaryInvoiceData?.billingperiodstart) ||
        getEffectiveInvoiceDate(invoice, "rental");
    if (!startDate) {
        return null;
    }
    const endDate = parseDateValue(invoice?.billingperiodend) ||
    ...
    return {
        ...
        label: normalizeText(invoice?.billingperiodlabel) ||
            normalizeText(summaryInvoiceData?.billingperiodlabel) ||
            `${formatDisplayDate(startDate)} - ${formatDisplayDate(endDate)}`,
    };
};
const getCustomerName = (customer, fallbackName = "") => {
...
const getInvoiceItemOrderLineIds = (invoice) => {
...
const getRentalProductSubcategory = (invoice) => {
    const invoiceData = parseJsonValue(invoice?.invoicedata, {});
    const summaryInvoiceData = parseJsonValue(invoice?.summaryinvoicedata, {});
    const items = Array.isArray(invoiceData?.items) ? invoiceData.items : [];
    const firstItem = items[0] || {};
    return firstPresentText(invoice?.productsubcategory, invoice?.productsubcategories, summaryInvoiceData?.productsubcategory, summaryInvoiceData?.productsubcategories, summaryInvoiceData?.subcategory, firstItem?.productsubcategory, firstItem?.productSubcategory, firstItem?.subcategory, inferProductSubcategoryFromText(summaryInvoiceData?.summarydescription), inferProductSubcategoryFromText(firstItem?.name || firstItem?.productname), inferProductSubcategoryFromText(invoice?.productname));
};
const getRentalProductSubcategoryLabel = (invoice) => firstPresentText(invoice?.productsubcategorylabel, invoice?.productSubcategoryLabel, formatProductSubcategoryLabel(getRentalProductSubcategory(invoice)));
const getRentalItemLabel = (invoice) => {
...
const getDescription = (invoice, invoiceFor, periodLabel) => {
    const invoiceData = parseJsonValue(invoice?.invoicedata, {});
    const summaryInvoiceData = parseJsonValue(invoice?.summaryinvoicedata, {});
    const items = Array.isArray(invoiceData?.items) ? invoiceData.items : [];
    if (invoiceFor === "rental") {
        const deviceCount = Math.max(toNumber(invoice?.rentaldevicecount) ||
            items.reduce((total, item) => total + Math.max(toNumber(item?.quantity), 1), 0), 1);
        const deviceLabel = deviceCount === 1 ? "Device" : "Devices";
        return (normalizeText(summaryInvoiceData?.summarydescription) ||
...
const getQuantityLabel = (invoice, invoiceFor) => {
    const invoiceData = parseJsonValue(invoice?.invoicedata, {});
    const items = Array.isArray(invoiceData?.items) ? invoiceData.items : [];
    if (invoiceFor === "rental") {
        const deviceCount = Math.max(toNumber(invoice?.rentaldevicecount) ||
    ...
    const quantity = Math.max(items.reduce((total, item) => total + Math.max(toNumber(item?.quantity), 1), 0), 1);
    return String(quantity);
};
const getInvoiceAmounts = (invoice) => {
    const invoiceData = parseJsonValue(invoice?.invoicedata, {});
    const summaryInvoiceData = parseJsonValue(invoice?.summaryinvoicedata, {});
    const taxMode = normalizeComparable(summaryInvoiceData?.taxmode || invoiceData?.taxmode || invoice?.taxmode);
    const totalAmount = toNumber(invoice?.totalorderamount) ||
    ...
    const explicitTaxableAmount = toNumber(invoiceData?.taxableamount) ||
        toNumber(invoiceData?.taxablevalue) ||
        toNumber(summaryInvoiceData?.taxablevalue);
    const payableTaxableAmount = totalAmount > 0 ? Math.max(totalAmount - taxAmount, 0) : 0;
    const taxableAmount = payableTaxableAmount || explicitTaxableAmount || 0;
    const explicitIgstAmount = toNumber(summaryInvoiceData?.igstamount) || toNumber(invoiceData?.igstamount);
    const explicitCgstAmount = toNumber(summaryInvoiceData?.cgstamount) || toNumber(invoiceData?.cgstamount);
    const explicitSgstAmount = toNumber(summaryInvoiceData?.sgstamount) || toNumber(invoiceData?.sgstamount);
...
const getSacCode = (invoice) => {
    const invoiceData = parseJsonValue(invoice?.invoicedata, {});
    const summaryInvoiceData = parseJsonValue(invoice?.summaryinvoicedata, {});
    const items = Array.isArray(invoiceData?.items) ? invoiceData.items : [];
    const firstItem = items[0] || {};
    return firstPresentText(summaryInvoiceData?.saccode, firstItem?.sacCode, firstItem?.saccode, firstItem?.hsncode, invoice?.saccode, invoice?.hsncode, "997315");
};
const getInvoiceTypeLabel = (invoiceFor) => {
    const labels = {
        rental: "Rental",
        service: "Service",
        penalty: "Penalty",
...
const buildCustomerAddress = (customer, invoiceRows) => {
    const addressParts = [
        customer?.doornumber,
        customer?.landmark,
        customer?.address,
...
const enrichRentalInvoiceProductMetadata = async (invoiceRows) => {
    const rentalInvoices = invoiceRows.filter((invoice) => normalizeComparable(invoice?.invoicefor) === "rental");
    const orderIds = Array.from(new Set(rentalInvoices
    ...
    return invoiceRows.map((invoice) => {
        if (normalizeComparable(invoice?.invoicefor) !== "rental")
            return invoice;
        ...
        const matchingLines = selectedOrderLineIds.length > 0
            ? orderLines.filter((line) => selectedOrderLineIdSet.has(Number(line?.orderlineid)))
            : orderLines;
        const subcategoryValues = Array.from(new Set(matchingLines
            .map((line) => normalizeComparable(line?.productsubcategory))
            .filter(Boolean))).sort();
        const productSubcategory = subcategoryValues.length === 1
            ? subcategoryValues[0]
        ...
        return {
            ...
            productsubcategorylabel: subcategoryValues.length > 1
                ? "Mixed Rental"
                : formatProductSubcategoryLabel(productSubcategory),
        };
    });
};
const fetchCandidateInvoices = async (customerId, invoiceForValues) => {
    const result = await query(`
    SELECT *
    FROM revoinvoice
    WHERE customerid = $1
...
const fetchPreviousBillingProgress = async (customerId, period, invoiceForValues) => {
    const result = await query(`
    SELECT
      cis.revoinvoiceid,
...
const buildProratedRentalRow = (invoice, period, billingThroughDate, previousBillingProgress) => {
    ...
    if (isAfterDate(billingPeriod.startDate, requestedPeriodEnd) ||
        isAfterDate(requestedPeriodStart, billingPeriod.endDate)) {
        return null;
    }
    const billingStartDate = maxDate(billingPeriod.startDate, requestedPeriodStart, nextUnbilledStart);
    ...
    return {
        ...
        invoicedate: formatDisplayDate(parseDateValue(invoice?.invoicedate) ||
            getEffectiveInvoiceDate(invoice, "rental")),
...
const extractSourceInvoiceEntry = (row) => {
...
const buildGroupedRentalDescription = (row, quantity, periodLabel) => {
...
const getGroupDateBoundary = (left, right, picker) => {
    const dates = [parseDateValue(left), parseDateValue(right)].filter(Boolean);
    return dates.length ? formatDateKey(picker(...dates)) : firstPresentText(left, right);
};
const refreshGroupedRentalLabels = (row, periodLabel) => {
    const sourceRows = Array.isArray(row.sourceinvoices) && row.sourceinvoices.length
        ? row.sourceinvoices
        : [row];
    const quantity = Math.max(sourceRows.reduce((total, sourceRow) => total + getRowQuantity(sourceRow), 0), 1);
    const uniqueBillingDays = Array.from(new Set(sourceRows.map((sourceRow) => normalizeText(sourceRow.billingDaysLabel)).filter(Boolean)));
    const uniqueBillingRanges = Array.from(new Set(sourceRows.map((sourceRow) => normalizeText(sourceRow.billingRangeLabel)).filter(Boolean)));
    const uniqueCycleRanges = Array.from(new Set(sourceRows.map((sourceRow) => normalizeText(sourceRow.cycleRangeLabel)).filter(Boolean)));
    ...
    row.invoicenumber =
        sourceRows.length === 1
            ...
            : sourceRows.map((sourceRow) => sourceRow.invoicenumber).join(", ");
    ...
    row.hasSupportingDocument = sourceRows.some((sourceRow) => sourceRow.hasSupportingDocument);
    return row;
};
const mergeRentalRows = (target, source, periodLabel) => {
    const sumFields = [
        "taxableamount",
        "taxamount",
        "totalamount",
    ...
    target.billingstartdate = getGroupDateBoundary(target.billingstartdate, source.billingstartdate, minDate);
    target.billingthroughdate = getGroupDateBoundary(target.billingthroughdate, source.billingthroughdate, maxDate);
    target.billingperiodstart = getGroupDateBoundary(target.billingperiodstart, source.billingperiodstart, minDate);
    target.billingperiodend = getGroupDateBoundary(target.billingperiodend, source.billingperiodend, maxDate);
    target.sourceinvoices.push(extractSourceInvoiceEntry(source));
    target.sourceinvoiceids = Array.from(new Set([
        ...(target.sourceinvoiceids || []),
        Number(source.sourceinvoiceid),
    ].filter((id) => Number.isFinite(id) && id > 0)));
    return refreshGroupedRentalLabels(target, periodLabel);
};
const consolidatePreviewRows = (rows, period) => {
    const groupedRows = [];
    ...
    rows.forEach((row) => {
        ...
        const productSubcategory = normalizeComparable(row.productsubcategory) ||
        ...
        if (!existingRow) {
            const groupedRow = {
                ...
                sourceinvoiceids: [Number(row.sourceinvoiceid)].filter((id) => Number.isFinite(id) && id > 0),
            };
...
const buildPreviewRows = (invoiceRows, period, billingThroughDate, previousBillingProgress, sourceInvoiceIds) => {
    const sourceIdSet = sourceInvoiceIds && sourceInvoiceIds.length > 0
    ...
    const rawRows = invoiceRows
        ...
        if (invoiceFor === "rental") {
            const rentalRow = buildProratedRentalRow(invoice, period, billingThroughDate, previousBillingProgress);
            if (!rentalRow)
                return null;
            return rentalRow;
        }
        ...
        const amounts = getInvoiceAmounts(invoice);
        const billingPeriodLabel = normalizeText(invoice?.billingperiodlabel) || period.periodlabel;
        return {
...
const sumRowAmounts = (rows, fieldName) => fromPaise(rows.reduce((total, row) => total + toPaise(row?.[fieldName]), 0));
...
const getRowsBillingRangeLabel = (rows, fallbackLabel) => {
    const rangeLabels = Array.from(new Set(rows.map((row) => normalizeText(row.billingRangeLabel)).filter(Boolean)));
...
const getConsolidatedSummaryDescription = (rows, periodLabel) => {
    ...
    const deviceCount = Math.max(rows.reduce((total, row) => {
        const match = normalizeText(row.quantityLabel).match(/(\d+(?:\.\d+)?)/);
...
const getTemplateTaxMode = (rows, totals) => totals.igstamount > 0 || rows.some((row) => row.taxmode === "igst")
    ? "igst"
    : "cgst_sgst";
const getSourceInvoiceRows = (rows) => rows.flatMap((row) => Array.isArray(row?.sourceinvoices) && row.sourceinvoices.length > 0
    ? row.sourceinvoices
    : [row]);
const getSourceInvoiceIds = (rows) => Array.from(new Set(getSourceInvoiceRows(rows)
    .map((row) => Number(row.sourceinvoiceid))
    .filter((id) => Number.isFinite(id) && id > 0)));
const getSourceInvoiceKey = (rows) => getSourceInvoiceRows(rows)
    .map((row) => {
    const sourceInvoiceId = Number(row.sourceinvoiceid);
    if (!Number.isFinite(sourceInvoiceId) || sourceInvoiceId <= 0)
        return "";
    return [
        sourceInvoiceId,
    ...
    .sort()
    .join(",");
const toVersionNumber = (record) => {
    const versionNumber = Number(record?.versionnumber);
    return Number.isFinite(versionNumber) && versionNumber > 0 ? versionNumber : 1;
};
const getCurrentConsolidatedInvoice = (records) => records.find((record) => record?.iscurrent === true || record?.iscurrent === "true") ||
    records[0] ||
    null;
const getNextVersionNumber = (records) => Math.max(0, ...records.map((record) => toVersionNumber(record))) + 1;
const isRoundOffReadyConsolidatedInvoice = (record) => {
    const metadata = parseJsonValue(record?.metadatajson, {});
    return (toNumber(metadata?.calculationVersion) >= CONSOLIDATED_INVOICE_CALCULATION_VERSION ||
        metadata?.templateData?.roundOffAmount != null ||
        metadata?.preview?.totals?.payableamount != null);
};
const fetchExistingConsolidatedInvoices = async (customerId, period, invoiceForKey) => {
    const result = await query(`
    SELECT *
...
const fetchExactExistingConsolidatedInvoice = async (customerId, period, invoiceForKey, sourceInvoiceKey) => {
    const result = await query(`
    SELECT *
    FROM consolidated_invoices
    WHERE customerid = $1
...
const getDefaultLogoDataUrl = async () => {
    ...
    try {
        ...
        console.warn(`Consolidated invoice logo not found at ${DEFAULT_LOGO_PATH}: ${error?.message || error}`);
    }
    return cachedDefaultLogoDataUrl;
};
...
const buildTemplateData = async (documentNumber, versionNumber, customer, customerAddress, period, rows, totals) => {
...
const buildPreview = async (requestBody) => {
    ...
    const previousBillingProgress = await fetchPreviousBillingProgress(customerId, period, invoiceForValues);
    const { rows, warnings } = buildPreviewRows(invoiceRows, period, billingThroughDate, previousBillingProgress, sourceInvoiceIds);
    const totals = buildTotals(rows);
    const sourceInvoiceKey = getSourceInvoiceKey(rows);
    const existingConsolidatedInvoices = await fetchExistingConsolidatedInvoices(customerId, period, invoiceForKey);
    const currentConsolidatedInvoice = getCurrentConsolidatedInvoice(existingConsolidatedInvoices);
    const matchingConsolidatedInvoice = existingConsolidatedInvoices.find((record) => normalizeText(record?.sourceinvoicekey) === sourceInvoiceKey &&
        isRoundOffReadyConsolidatedInvoice(record)) || null;
    const hasCurrentDifferentSources = Boolean(rows.length > 0 &&
        sourceInvoiceKey &&
    ...
    return {
        customer: {
            ...
            phone: normalizeText(customer.usermobilenumber) ||
                normalizeText(customer.addressmobilenumber),
...
export var consolidatedInvoiceService;
(function (consolidatedInvoiceService) {
    consolidatedInvoiceService.listConsolidatedInvoices = async (requestBody) => {
        const customerId = Number(requestBody?.customerid);
        if (!Number.isFinite(customerId) || customerId <= 0) {
            throw new Error("A valid customer id is required.");
        }
    ...
    consolidatedInvoiceService.previewConsolidatedInvoice = async (requestBody) => {
        return buildPreview(requestBody);
    };
    consolidatedInvoiceService.generateConsolidatedInvoice = async (request) => {
        ...
        const exactExisting = await fetchExactExistingConsolidatedInvoice(customerId, period, preview.invoiceforkey, preview.sourceinvoicekey);
        const shouldRegenerateLegacyRoundOff = Boolean(exactExisting && !isRoundOffReadyConsolidatedInvoice(exactExisting));
        ...
        const hasCurrentDifferentSources = Boolean(currentConsolidatedInvoice &&
        ...
        const revisionReason = normalizeText(request.body?.revisionreason) ||
            (hasCurrentDifferentSources
      ...
      `, [
        ...
        const templateData = await buildTemplateData(documentNumber, versionNumber, preview.customer, preview.customer.address, period, preview.rows, preview.totals);
        ...
        `, [
            ...
            for (const row of getSourceInvoiceRows(preview.rows)) {
                await client.query(`
        ...
        const updatedExistingConsolidatedInvoices = [
            generatedRecord,
            ...preview.existingConsolidatedInvoices.map((record) => record?.iscurrent === true || record?.iscurrent === "true"
...
})(consolidatedInvoiceService || (consolidatedInvoiceService = {}));
//# sourceMappingURL=consolidatedInvoice.service.js.map