...
class ShiprocketShippingService {
    ...
    async diagnoseAccount() {
        try {
            ...
            try {
                const profileResponse = await axios.get(`${process.env.SHIPROCKET_BASE_URL}/settings/company/profile`, {
            ...
            try {
                const courierResponse = await axios.get(`${process.env.SHIPROCKET_BASE_URL}/courier/courierList`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            ...
            try {
                const serviceResponse = await axios.get(`${process.env.SHIPROCKET_BASE_URL}/courier/serviceability/`, {
                    headers: { Authorization: `Bearer ${token}` },
                    params: {
    ...
    async getLowestShippingCost(deliveryPincode, weight = 1, cod = false) {
        try {
    ...
    async getAllShippingOptions(deliveryPincode, weight = 1, cod = false) {
