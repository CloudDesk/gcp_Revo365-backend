...
import { FinanceValidationError, formatTdsSectionDisplayName, nowEpoch, resolveFinanceContext, } from "../utils/finance/finance.utils.js";
...
const normalizePayload = (body) => ({
    ...
    natureofpayment: requiredText(body?.natureofpayment, "natureofpayment", 500),
    rate: requiredText(body?.rate, "rate", 50),
});
const withDisplayName = (row) => ({
    ...row,
    displayname: formatTdsSectionDisplayName(row.natureofpayment, row.newcode, row.rate),
});
const requireSectionId = (request) => {
...
export var tdsSectionService;
(function (tdsSectionService) {
    tdsSectionService.list = async (request) => {
    ...
    tdsSectionService.getById = async (request) => {
        const { organizationId } = resolveFinanceContext(request);
      ...
      `, [sectionId, organizationId]);
        if (!result.rows[0]) {
            throw new FinanceValidationError("TDS section was not found.", 404, "TDS_SECTION_NOT_FOUND");
        }
        return withDisplayName(result.rows[0]);
    };
    tdsSectionService.create = async (request) => {
    ...
    tdsSectionService.update = async (request) => {
        const { actor, organizationId } = resolveFinanceContext(request);
        const sectionId = requireSectionId(request);
        const data = normalizePayload(request.body || {});
        const result = await query(`
...
})(tdsSectionService || (tdsSectionService = {}));
//# sourceMappingURL=tdsSection.service.js.map