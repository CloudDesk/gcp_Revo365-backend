...
const calculateMonthDelta = (currentEndDate, renewedEndDate) => {
    let monthDelta = (renewedEndDate.getUTCFullYear() - currentEndDate.getUTCFullYear()) * 12 +
...
export const processRentalRenewal = async ({ executor, context, ticketId, requestedRenewalDate, approvedRenewalDate = null, remarks = null, createdBy = null, }) => {
    ...
    if (approvedEpoch < currentEndEpoch) {
        throw new Error("Renewal date cannot be earlier than the current contract end date.");
    }
    ...
    const agreementId = linkedOrderline?.agreementid ?? context?.ticket?.agreementid ?? null;
    ...
    `, [
        ...
        Math.max(renewedRentalFor - Number(linkedOrderline?.generatedmonthscount ?? 0), 0),
        remarks,
        createdBy,
        JSON.stringify({
...
//# sourceMappingURL=rentalRenewal.service.js.map