...
import { assertSupplierBillCanBeModified, assertSupplierBillTotalWithinPurchaseOrder, validateSupplierBillProductInput, } from "../utils/finance/supplierBill.utils.js";
...
(function (poinvoiceservice) {
    ...
    const getBillTransactionState = async (id) => {
    ...
    const assertBillHasNoTransactions = async (id) => {
        assertSupplierBillCanBeModified(await getBillTransactionState(id));
    };
    const resolveBillStatus = (balanceAmount, isCreditPayment, paymentDueDate) => {
        const dueDate = Number(paymentDueDate);
        const isOverdue = isTrue(isCreditPayment) &&
    ...
    const applyCashAccountSettlementState = async (upsertFields, id) => {
        if (!id) {
            const invoiceAmount = toNumber(upsertFields.invoiceamount);
            upsertFields.paymentdata = [];
            upsertFields.balanceamount = invoiceAmount;
            upsertFields.invoicestatus = resolveBillStatus(invoiceAmount, upsertFields.iscreditpayment, upsertFields.paymentduedate);
            return;
        }
        const existingResult = await query(`SELECT invoiceamount, balanceamount, iscreditpayment, paymentduedate
       FROM poinvoice
       WHERE id = $1`, [id]);
        ...
        const invoiceAmount = Object.prototype.hasOwnProperty.call(upsertFields, "invoiceamount")
            ? toNumber(upsertFields.invoiceamount)
            : toNumber(existingBill.invoiceamount);
        const settledAmount = Math.max(toNumber(existingBill.invoiceamount) -
            toNumber(existingBill.balanceamount), 0);
        if (invoiceAmount < settledAmount) {
            throw new Error(`Bill amount cannot be less than the settled Cash and Bank amount ${settledAmount}`);
        }
    ...
    const validateAndNormalizeProductData = async (upsertFields, id) => {
        ...
        billProducts.forEach((product, index) => {
            ...
            const { productName, quantity } = validateSupplierBillProductInput(product?.name, product?.quantity, purchaseOrderProduct.name);
    ...
    const validateBillAmountWithinPurchaseOrder = async (upsertFields, id) => {
       ...
       LIMIT 1`, [ponumber]);
        ...
        const existingBillResult = await query(`SELECT COALESCE(SUM(invoiceamount), 0) AS total
       FROM poinvoice
       WHERE ${existingWhere}`, existingParams);
        ...
        assertSupplierBillTotalWithinPurchaseOrder(purchaseOrderTotal, existingBillTotal, currentBillTotal);
    };
