...
(function (dashboardservice) {
    dashboardservice.getSalesPerMonthData = async (querydata) => {
        try {
            ...
            fields.forEach((key, index) => {
                if (fieldMappings[key]) {
                    ...
                    if (!isValidMonth(values[index])) {
                        throw new Error('Invalid month value');
                    }
                    conditions.push(`TO_CHAR(TO_TIMESTAMP(o.createddate ), 'Month') ILIKE $${index + 1}`);
                    queryParams.push(`%${values[index].charAt(0).toUpperCase() + values[index].slice(1).toLowerCase()}%`);
                    if (!querydata.year) {
    ...
    dashboardservice.getSalesMonthlyData = async (querydata) => {
        try {
...
ORDER BY
    months.month
`;
            const result = await query(queryText, [startTimestamp, endTimestamp]);
            await dataTypeCheck(result);
...
`;
    ...
    dashboardservice.getGroupbyValueData = async (querydata) => {
        try {
            const getColumns = async (tableName) => {
                const columnQuery = `
                SELECT column_name
            ...
            for (const [field, values] of Object.entries(querydata)) {
                if (field === 'data')
                    continue; // skip date range parameter
    ...
    dashboardservice.getDynamicGroupbyValueData = async (querydata) => {
        try {
            const getColumns = async (tableName) => {
                const columnQuery = `
                SELECT column_name
    ...
    dashboardservice.getDynamicGroupbyValueData2 = async (querydata) => {
        try {
            const getColumns = async (tableName) => {
                const columnQuery = `
                SELECT column_name
    ...
    dashboardservice.getOrderStstusDashboardAmountQuantityData = async (querydata) => {
        try {
            ...
            if (statusesToInclude.length === allStatuses.length) {
                selectClauses.push(`COALESCE(SUM(o.orderamount), 0) AS "Total Amount"`, `COALESCE(SUM(o.quantity), 0) AS "Total Quantity"`);
            }
            let queryText = `
                WITH date_series AS (
    ...
    dashboardservice.getOrderStstusDashboardQuantityData = async (querydata) => {
        try {
            ...
            if (statusesToInclude.length === allStatuses.length) {
                selectClauses.push(`COALESCE(SUM(ol.quantity), 0) AS "Total Quantity"`);
            }
            let queryText = `
                WITH date_series AS (
                    SELECT generate_series(
    ...
    dashboardservice.getCountDashboardData = async (querydata, params) => {
        try {
...
`;
            ...
            const formattedResult = result.rows.map(row => {
                ...
                for (const [key, value] of Object.entries(row)) {
                    if (key === 'Month' || key === 'Year') {
                        ...
                        newRow[formatColumnName(key)] = parseInt(value, 10);
                    }
                }
                return newRow;
            });
    ...
    dashboardservice.getTicketCountDashboardData = async (querydata) => {
        try {
            ...
            const formatColumnName = (name) => name
                ...
                .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                .join(' ');
            const selectClauses = statusesToInclude.map((status, i) => `COUNT(CASE WHEN o.ticketstatus = $${i + 3} THEN 1 ELSE NULL END) AS "${formatColumnName(status)} Count"`);
            if (statusesToInclude.length === allStatuses.length) {
    ...
    dashboardservice.getEpochTicketCountDashboardData = async (querydata) => {
        try {
    ...
    dashboardservice.getTechnicianTicketCountDashboardData = async (querydata) => {
        try {
    ...
    dashboardservice.getProductStatusCountDashboardData = async (querydata) => {
        try {
            ...
            if (Array.isArray(productstatus)) {
                ...
                selectClauses = validStatuses.map(status => `COUNT(id) FILTER (WHERE productstatus = '${status}') AS ${status}`);
            }
            else if (productstatus === 'all') {
                selectClauses = allStatuses.map(status => `COUNT(id) FILTER (WHERE productstatus = '${status}') AS ${status}`);
            }
    ...
    dashboardservice.getTodayTicketPriorityCountData = async (querydata) => {
        try {
            ...
            if (Array.isArray(ticketpriority)) {
                ...
                selectClauses = validPriorities.map(p => `COUNT(id) FILTER (WHERE date_trunc('day', to_timestamp(createddate)) = date_trunc('day', CURRENT_TIMESTAMP) AND lower(ticketpriority) = '${p}') AS ${p}`);
            }
            else if (ticketpriority === 'all') {
                selectClauses = allPriorities.map(p => `COUNT(id) FILTER (WHERE date_trunc('day', to_timestamp(createddate)) = date_trunc('day', CURRENT_TIMESTAMP) AND lower(ticketpriority) = '${p}') AS ${p}`);
    ...
    dashboardservice.getTodayTicketTypeCountData = async (querydata) => {
        try {
            ...
            if (Array.isArray(tickettype)) {
                ...
                selectClauses = validTypes.map(p => `COUNT(id) FILTER (WHERE date_trunc('day', to_timestamp(createddate)) = date_trunc('day', CURRENT_TIMESTAMP) AND lower(tickettype) = '${p.toLowerCase()}') AS "${p}"`);
            }
            else if (tickettype === 'all') {
                selectClauses = allTypes.map(p => `COUNT(id) FILTER (WHERE date_trunc('day', to_timestamp(createddate)) = date_trunc('day', CURRENT_TIMESTAMP) AND lower(tickettype) = '${p.toLowerCase()}') AS "${p}"`);
                selectClauses.push(`COUNT(id) FILTER (WHERE date_trunc('day', to_timestamp(createddate)) = date_trunc('day', CURRENT_TIMESTAMP)) AS "total"`);
            }
    ...
    const getStockValueTypeFilter = (querydata) => {
    ...
    dashboardservice.getAvailableCountTotalData = async (querydata = {}) => {
        try {
            ...
            const formattedResult = stockValueCategories.map(({ label, category, subcategory }) => {
                const row = result.rows.find(r => r.category.toLowerCase() === category &&
    ...
    dashboardservice.getBusinessCustomerRentalStockData = async () => {
        try {
            const result = await query(`
    ...
    dashboardservice.getAvailableCountData = async () => {
        try {
            const queryText = `
                SELECT
    ...
    dashboardservice.getAvailableCountDatalocation = async (querydata) => {
        try {
            const { location } = querydata;
            if (!location) {
                throw new Error('Location parameter is required.');
            ...
            const formattedResult = categories.map(category => {
                ...
                const row = result.rows.find(r => r.category.toLowerCase() === cat &&
    ...
    dashboardservice.getInvoiceData = async () => {
        try {
            const queryText = `
                SELECT 
                    p.ponumber,
    ...
    dashboardservice.getInvoiceDataDateBased = async (querydata) => {
        try {
            let { date } = querydata;
    ...
    dashboardservice.getSoldDetailsData = async (querydata) => {
        try {
