...
(function (stockRevoService) {
    ...
    const validateStockPurchaseDetails = async (stockData, required) => {
        ...
        const supplierResult = await query(`SELECT id FROM supplier WHERE id = $1 LIMIT 1`, [supplierId]);
    ...
    const getDuplicateBarcodeError = async (barcode, currentId) => {
    ...
    stockRevoService.generateUniqueBarcodeNumber = async () => {
        const result = await query(`
                WITH used AS (
    ...
    const getManualStockStatusTransitionError = (currentStatus, requestedStatus) => {
        ...
        if (next === normalizeComparableText(AVAILABLE_STOCK_STATUS) && (current === normalizeComparableText(DAMAGED_STOCK_STATUS)
    ...
    stockRevoService.searchStockRevoData = async (request) => {
        try {
            const searchValue = String(request.query?.search ?? "").trim();
            const pageNumber = Math.max(parseInt(request.query?.page) || 1, 1);
            const recordCount = Math.min(Math.max(parseInt(request.query?.count) || 10, 1), 5000);
            if (!searchValue) {
                return [];
            }
            ...
            Object.entries(supportedFilters).forEach(([filterName, columnName]) => {
                ...
                const populatedValues = filterValues.filter((value) => value !== undefined && value !== null && value !== "");
            ...
            whereClauses.push(`(s.isarchive = FALSE OR s.isarchive IS NULL)`, `(s.isdeleted = FALSE OR s.isdeleted IS NULL)`, `(s.removefromrecyclebin = FALSE OR s.removefromrecyclebin IS NULL)`);
            const offset = (pageNumber - 1) * recordCount;
            queryParams.push(offset, recordCount);
    ...
    stockRevoService.upsertStockRevoData = async (stockRevoData) => {
        try {
            let querydata;
            let params;
            const purchaseDetailsValidationError = await validateStockPurchaseDetails(stockRevoData, !stockRevoData.id);
            ...
            if (id) {
                ...
                if (oldStockResult.rows.length > 0) {
                    ...
                    const statusTransitionError = getManualStockStatusTransitionError(currentRow.stockstatus, upsertFields.stockstatus);
                ...
                if (!upsertFields.rfid) {
                    const generatedBarcode = await stockRevoService.generateUniqueBarcodeNumber();
    ...
    stockRevoService.releaseServiceHoldStockToAvailable = async (stockId, modifiedBy) => {
        try {
            if (!stockId) {
    ...
    stockRevoService.markLostStockAsFound = async (stockId, modifiedBy) => {
        try {
            if (!stockId) {
    ...
    stockRevoService.upsertBulkStockRevoData = async (jsonresult) => {
        try {
            ...
            for (let i = 0; i < jsonresult.length; i++) {
                const purchaseDetailsValidationError = await validateStockPurchaseDetails(jsonresult[i], true);
                if (purchaseDetailsValidationError) {
                    return {
    ...
    stockRevoService.upsertStockRevoDatarfid = async (rfidDataArray) => {
        try {
            ...
            if (!ordername && rfidDataArray[0]?.orderlinenumber) {
                const orderlineQuery = await query(`SELECT ordername FROM orderline WHERE orderlinenumber = $1 LIMIT 1`, [rfidDataArray[0].orderlinenumber]);
