...
const normalizeQuotationType = (value) => {
...
const normalizeVersionStatus = (value, fallback = "draft") => {
    const normalized = String(value || fallback).toLowerCase().trim();
    const allowed = new Set(["draft", "sent", "revised", "accepted", "rejected", "expired"]);
    return allowed.has(normalized) ? normalized : fallback;
};
const buildQuotationNumber = (id) => `SQ-${String(id).padStart(6, "0")}`;
const normalizeVersionPayload = (data) => ({
...
const formatWhereFilters = (queryParamsObject) => {
...
export var storeQuotationService;
(function (storeQuotationService) {
    storeQuotationService.getStoreQuotations = async (request) => {
        try {
    ...
    storeQuotationService.getStoreQuotationVersions = async (request) => {
        try {
            const quotationId = request.params?.id || request.query?.quotationid;
            const result = await query(`SELECT * FROM store_quotation_versions WHERE quotationid = $1 ORDER BY versionnumber DESC`, [quotationId]);
            return await dataTypeCheck(result);
        }
        catch (error) {
    ...
    storeQuotationService.upsertStoreQuotation = async (quotationData) => {
        ...
        try {
          ...
          `, [
                ...
                const numberResult = await client.query(`UPDATE store_quotations SET quotationnumber = $1 WHERE id = $2 RETURNING *`, [quotationNumber, quotationId]);
                headerRow = numberResult.rows[0];
            }
            if (!headerRow) {
                throw new Error("Quotation not found");
            }
            const versionNumberResult = await client.query(`SELECT COALESCE(MAX(versionnumber), 0) + 1 AS nextversion FROM store_quotation_versions WHERE quotationid = $1`, [quotationId]);
        ...
        `, [
            ...
            const refreshedHeader = await client.query(`SELECT * FROM store_quotations WHERE id = $1`, [quotationId]);
    ...
    storeQuotationService.finalizeStoreQuotation = async (request) => {
        try {
    ...
    storeQuotationService.markStoreQuotationConverted = async (data) => {
        try {
    ...
    storeQuotationService.updateStoreQuotationUrl = async (data) => {
        try {
            const quotationId = data?.quotationid || data?.id;
...
})(storeQuotationService || (storeQuotationService = {}));
//# sourceMappingURL=storeQuotation.service.js.map