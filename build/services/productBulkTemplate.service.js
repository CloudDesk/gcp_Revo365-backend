...
const CORE_FIELD_SET = new Set(CORE_BULK_UPLOAD_FIELDS);
...
const DEFAULT_TEMPLATE_PROFILE = "legacy_product_bulk";
...
const getTypeList = (property) => {
    if (Array.isArray(property.type)) {
        return property.type.filter((item) => typeof item === "string");
    }
...
const buildConstraintsLabel = (property, filteredTypes) => {
    const constraints = [];
    ...
    if (primaryType === "array" && property.items?.type) {
        const arrayType = Array.isArray(property.items.type)
            ? property.items.type.filter((item) => typeof item === "string").join(" | ")
            : property.items.type;
        constraints.push(`items: ${arrayType}`);
    }
    return constraints.join(" | ");
};
const getFallbackExample = (fieldKey, filteredTypes) => {
    const primaryType = filteredTypes[0];
...
const resolveTemplateProfile = (requestedProfile) => {
    const profileKey = (requestedProfile || DEFAULT_TEMPLATE_PROFILE);
...
const buildTemplateFields = (profile) => {
    const schema = productrevoInsertSchema;
    const properties = schema.properties ?? {};
    const requiredFromSchema = new Set(schema.required ?? []);
    const priorityFieldRank = new Map((profile.fields === "ALL" ? CORE_BULK_UPLOAD_FIELDS : profile.fields).map((fieldName, index) => [
        fieldName,
        index,
    ]));
    const selectedFields = profile.fields === "ALL" ? null : new Set(profile.fields.filter((fieldName) => fieldName in properties));
    return Object.entries(properties)
        .filter(([key]) => {
        if (FIELD_OVERRIDES[key]?.include === false || EXCLUDED_TEMPLATE_FIELDS.has(key))
            return false;
        if (!selectedFields)
            return true;
...
const addTemplateSheet = (workbook, fields) => {
...
const buildValidationRangeFormula = (sheetName, columnLetter, rowCount) => {
    if (rowCount <= 0)
        return null;
    const endRow = Math.max(2, rowCount + 1);
    return `'${sheetName}'!$${columnLetter}$2:$${columnLetter}$${endRow}`;
};
const applyReferenceDataValidations = (templateSheet, fields, referenceRowCount) => {
...
const fetchSupplierPoReferenceData = async () => {
    const result = await query(`SELECT po.supplierid, s.suppliername, po.ponumber, po.po_status
...
const addSupplierPoReferenceSheet = (workbook, referenceData) => {
...
const addInstructionSheet = (workbook, fields, profile) => {
...
export var productBulkTemplateService;
(function (productBulkTemplateService) {
    productBulkTemplateService.generateProductBulkTemplate = async (requestedProfile) => {
    ...
    productBulkTemplateService.defaultProfile = DEFAULT_TEMPLATE_PROFILE;
    productBulkTemplateService.supportedProfiles = Object.keys(TEMPLATE_PROFILES);
})(productBulkTemplateService || (productBulkTemplateService = {}));
//# sourceMappingURL=productBulkTemplate.service.js.map