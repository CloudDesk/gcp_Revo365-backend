...
import { FinanceValidationError, calculateAvailableBalance, nowEpoch, requirePositiveMoney, toFinanceDateOnly, toMoney, } from "../utils/finance/finance.utils.js";
...
const isMatchingInvoicePayment = (payment, transaction) => {
...
const getEligibleOrderContext = async (merchantTransactionId) => {
    ...
    if (!isEligibleEcommerceOrder(orderRows))
        return null;
...
const markEventFailure = async (eventId, status, code, message) => {
    await query(`
...
const linkInvoiceToPostedReceipt = async (invoiceId) => {
    ...
    try {
      ...
      `, [invoiceId]);
        ...
        if (String(invoice.invoicefor || "").trim().toLowerCase() !== "product" ||
        ...
        const merchantTransactionId = String(invoice.linkedmerchanttransactionid || "").trim();
      ...
      WHERE documenttype = 'sales_invoice'
        ...
        AND status = 'applied'
      `, [invoiceId]);
        const documentOutstanding = toMoney(Math.max(invoiceAmount - toMoney(allocatedResult.rows[0]?.allocatedamount), 0));
        const allocationAmount = toMoney(Math.min(toMoney(unapplied.remainingamount), documentOutstanding));
      ...
      `, [ORGANIZATION_ID]);
        ...
        const customerAdvanceAccountId = Number(bankTransaction.counterpartyaccountid);
        if (!Number.isSafeInteger(accountsReceivableId) ||
            !Number.isSafeInteger(customerAdvanceAccountId)) {
            throw new FinanceValidationError("The Accounts Receivable or Customer Advance ledger is unavailable.", 409, "ECOMMERCE_ALLOCATION_LEDGER_MISSING");
        }
      ...
      `, [
        ...
        const remainingAmount = toMoney(toMoney(unapplied.remainingamount) - allocationAmount);
        const appliedAmount = toMoney(toMoney(unapplied.appliedamount) + allocationAmount);
        await client.query(`
      ...
      `, [
        ...
        const matchingPaymentIndex = paymentData.findIndex((payment) => isMatchingInvoicePayment(payment, bankTransaction));
        const financePayment = {
            paymentamount: allocationAmount,
            paymentmethod: "online",
            paymentdate: Number(bankTransaction.posteddate || epoch),
            transactionreference: bankTransaction.sourcepaymentid || bankTransaction.transactionnumber,
        ...
        const paidAmount = toMoney(Math.min(invoiceAmount, nextPaymentData.reduce((total, payment) => String(payment?.status || "success").toLowerCase() === "failed"
            ? total
            : total + toInvoiceNumber(payment?.paymentamount), 0)));
        ...
        const paymentStatus = balanceAmount === 0
            ...
            : paidAmount > 0
                ? "partially_paid"
                : "pending";
        const lastPaymentDate = nextPaymentData.reduce((latest, payment) => Math.max(latest, toInvoiceNumber(payment?.paymentdate)), 0);
        await client.query(`
      UPDATE revoinvoice
...
const linkExistingInvoicesForPayment = async (merchantTransactionId) => {
...
const processEvent = async (eventId) => {
    ...
    try {
      ...
      `, [eventId]);
        ...
        if (!event) {
            throw new FinanceValidationError("E-commerce finance event was not found.", 404, "ECOMMERCE_FINANCE_EVENT_NOT_FOUND");
        }
      ...
      `, [bankCashAccountId, event.organizationid]);
        ...
        if (!bankCashAccount) {
            throw new FinanceValidationError("The e-commerce default Bank account is unavailable.", 409, "ECOMMERCE_DEFAULT_BANK_ACCOUNT_UNAVAILABLE");
        }
      ...
      `, [event.organizationid]);
        ...
        if (!Number.isSafeInteger(customerAdvanceAccountId)) {
            throw new FinanceValidationError("Customer Advances system ledger is unavailable.", 409, "CUSTOMER_ADVANCE_LEDGER_MISSING");
        }
      ...
      `, [bankCashAccountId]);
        ...
        if (latestTransactionDate &&
            ...
            throw new FinanceValidationError("The payment predates the latest posted Bank/Cash transaction. Backdated automatic posting is not enabled.", 409, "BACKDATED_FINANCE_POSTING_NOT_SUPPORTED");
        }
        const amount = requirePositiveMoney(event.amount);
        const balanceAfter = calculateAvailableBalance(bankCashAccount.currentbalance, "debit", amount);
        const epoch = nowEpoch();
      ...
      `, [
        ...
        const message = String(error?.message || "Unable to post the e-commerce payment.");
...
export var ecommercePaymentFinanceService;
(function (ecommercePaymentFinanceService) {
    ecommercePaymentFinanceService.recordSuccessfulPayment = async (transactionRow) => {
        const merchantTransactionId = String(transactionRow?.merchanttransactionid || "").trim();
        if (!merchantTransactionId) {
        ...
        const paymentMethod = resolveEcommercePaymentMethod(transactionRow, orderContext.orderRows);
        const paymentDate = resolveEcommercePaymentDate(transactionRow);
        const customerName = buildEcommerceCustomerName(orderContext.user, transactionRow);
        const epoch = nowEpoch();
      ...
      `, [
        ...
        const event = eventResult.rows[0] ||
          ...
          LIMIT 1
          `, [ORGANIZATION_ID, provider, sourcePaymentId])).rows[0];
        if (!event) {
            throw new FinanceValidationError("Unable to resolve the e-commerce finance event.", 500, "ECOMMERCE_FINANCE_EVENT_RESOLUTION_FAILED");
        }
    ...
    ecommercePaymentFinanceService.linkInvoice = async (invoiceId) => {
    ...
    ecommercePaymentFinanceService.safelyLinkInvoice = async (invoiceId) => {
        try {
            const result = await ecommercePaymentFinanceService.linkInvoice(invoiceId);
            if (result.status === "failed") {
                console.warn("[EcommerceFinance] Invoice allocation was not completed.", result);
            }
            return result;
        }
        catch (error) {
            console.error("[EcommerceFinance] Unable to link invoice to posted receipt:", error?.message || error);
            return {
                status: "failed",
                invoiceId,
    ...
    ecommercePaymentFinanceService.safelyRecordSuccessfulPayment = async (transactionRow) => {
        try {
            const result = await ecommercePaymentFinanceService.recordSuccessfulPayment(transactionRow);
            ...
            console.error("[EcommerceFinance] Unable to capture successful payment:", error?.message || error);
            return {
                status: "failed",
    ...
    ecommercePaymentFinanceService.processPendingPayments = async (limit = 100) => {
...
})(ecommercePaymentFinanceService || (ecommercePaymentFinanceService = {}));
//# sourceMappingURL=ecommercePaymentFinance.service.js.map