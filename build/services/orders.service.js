...
(function (ordersService) {
    ...
    const executeQuery = async (runner, stmt, params = []) => {
        if (runner?.query) {
            return await runner.query(stmt, params);
        }
        return await query(stmt, params);
    };
    const getOrderlineInsertableColumns = async (runner) => {
        if (orderlineColumnCache) {
            return orderlineColumnCache;
        }
        const result = await executeQuery(runner, `
            SELECT column_name
    ...
    const sendOrderCancellationEmail = async (userid, orderId, orderAmount) => {
        if (!userid)
            return;
...
${whereClause} ${orderByClause}`;
    ...
    const restoreInventoryForReturnedLines = async (lineRows, previousStatuses) => {
        ...
        if (preDispatchLines.length > 0) {
            await productrevoService.releaseCommittedQuantityForOrderLines(preDispatchLines.map((line) => ({
                merchanttransactionid: line.merchanttransactionid,
                productid: line.productid,
                quantity: line.quantity,
                ordername: line.ordername,
            ...
            await inventoryReservationService.transitionCommittedReservationsForOrderLines(preDispatchLines.map((line) => ({
                merchanttransactionid: line.merchanttransactionid,
                productid: line.productid,
                quantity: line.quantity,
                ordername: line.ordername,
                ordertype: line.ordertype,
                deliveryfrom: line.deliveryfrom,
            })), "released", "order_returned_pre_dispatch");
        }
    ...
    ordersService.handleReturnedOrderLines = async (lineRows, previousStatuses) => {
        ...
        try {
            ...
            if (newStatus === 'cancelled' && previousStatus !== 'cancelled') {
                ...
                await query(`UPDATE orderline
                 ...
                 AND COALESCE(orderstatus, '') NOT IN ('cancelled','delivered','returned','payment_failed')`, [updatedRow.orderid]);
        ...
        try {
            ...
            if (lineStatus === 'cancelled' && previousStatus !== 'cancelled') {
                ...
                if (lineRow?.ordertype === 'Orders') {
                    await productrevoService.updateCancelledOrderedQuantity([lineRow.productid], Number(lineRow.quantity));
                }
                if (orderType === "rental") {
            ...
            else if (["delivered", "sold"].includes(lineStatus) &&
   ...
   RETURNING id, rentalfor, generatedmonthscount, invoicegenerated, lastgeneratedinvoicedate`, [idsToUpdate]);
        ...
        `;
            ...
            if (updateStock && (updateStock.command === "UPDATE" || updateStock.command === "INSERT")) {
                ...
                if (!ordername && pucArray.length > 0) {
                    const stockTypeQuery = await query(`SELECT EXISTS (
                ...
                // ✅ Update quantity
                await stockRevoService.updateQuantity(pucArray, updateStock.result.rowCount, true, finalIsRental);
                const ordersToUpdate = updateStock.result.rows.filter(e => e.orderlinenumber);
                if (ordersToUpdate.length > 0) {
                    const rfidsByOrderLine = new Map();
                    ordersToUpdate.forEach((e) => {
                    ...
                    SET 
                        ...
                        assetnumber = CASE 
                            ${Array.from(rfidsByOrderLine.entries()).map(([ordNo, rfids]) => `WHEN orderlinenumber = '${ordNo}' THEN '${rfids.join(', ')}'`).join(' ')}
                            ELSE assetnumber
                        END,
                        deliveryfrom = CASE 
                ...
                `;
                    ...
                    // ✅ Sync order headers
                    await ordersService.syncOrderHeadersFromOrderLines(Array.from(new Set(result.rows.map((r) => r.uniqueorderid).filter(Boolean))));
                    return result;
                }
            }
    ...
    ordersService.bulkInsertOrder = async (transactionData, orderData) => {
        try {
            ...
            const billingAddressSnapshot = normalizeAddressSnapshot(orderData?.[0]?.billingaddresssnapshot ?? transactionData?.billingaddresssnapshot);
            const shippingAddressSnapshot = normalizeAddressSnapshot(orderData?.[0]?.shippingaddresssnapshot ?? transactionData?.shippingaddresssnapshot);
            ...
            try {
                ...
                `;
                    ...
                    if (orderResult.command === 'INSERT') {
                        ...
                        const orderlineResult = await ordersService.bulkInsertOrderlines(ordersToInsert, client);
                ...
                `;
                    ...
                    if (thirdPartyResult.command === 'INSERT') {
                        ...
                        const orderlineResult = await ordersService.bulkInsertOrderlines(thirdPartyOrdersToInsert, client);
                        if (orderlineResult.command !== 'INSERT' ||
    ...
    ordersService.bulkInsertOrderlines = async (orderData, runner) => {
        try {
            ...
            const sanitizedOrderData = (orderData || []).map((order) => Object.fromEntries(Object.entries(order || {}).filter(([field]) => allowedColumns.has(field))));
            ...
            const values = sanitizedOrderData.flatMap(order => fields.map(field => order[field]));
...
Thank You!`,
            ...
            let resolvedRecipient = isLikelyEmailAddress(emailid)
                ? String(emailid).trim()
                : null;
            const rentalOrderRows = result.rows.filter((row) => normalizeComparableText(row?.ordername) === "rental" ||
            ...
            const rentalOrderLogContext = {
                ...
                orderLineNumbers: rentalOrderRows.map((row) => row?.orderlinenumber).filter(Boolean),
                uniqueOrderIds: Array.from(new Set(rentalOrderRows.map((row) => row?.uniqueorderid).filter(Boolean))),
            };
            if (!resolvedRecipient) {
                const lineUserId = result.rows.find((row) => row?.userid)?.userid;
                if (lineUserId) {
                    const userResult = await query(`SELECT useremail FROM users WHERE id = $1 LIMIT 1`, [lineUserId]);
            ...
            try {
                if (resolvedRecipient) {
                    if (isRentalOrderEmail) {
                        console.log("[RentalOrderEmail] Sending rental order confirmation email", rentalOrderLogContext);
                    }
                    await sendTransactionalMail({
                        ...maildata.body,
                    ...
                    if (isRentalOrderEmail) {
                        console.log("[RentalOrderEmail] Rental order confirmation email sent successfully", rentalOrderLogContext);
                    }
                }
                else {
                    console.warn("Order email notification skipped: no valid recipient email found", { orderid, providedRecipient: emailid || null });
                    if (isRentalOrderEmail) {
                        console.warn("[RentalOrderEmail] Rental order confirmation email skipped: no valid recipient email found", {
                ...
                if (isRentalOrderEmail) {
                    console.error("[RentalOrderEmail] Rental order confirmation email failed", {
                        ...rentalOrderLogContext,
        ...
        try {
            ...
            for (const row of lockedProductRows.rows) {
                await query(`UPDATE product_revo
                     ...
                     WHERE id = $2`, [Number(row.quantity) || 0, row.productid]);
            }
