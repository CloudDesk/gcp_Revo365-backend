...
export module ErrorHandler {
    ...
    export const checkErrorMessage = async (error: PostgresError) => {
            ...
            switch (error.code) {
                case '23505': // Duplicate key violation
                    ...
                    if (error.detail) {
                        ...
                        if (match) {
                            errorDetails.push({ key: match[1], message: `${match[1]} ${match[2]}  already exists.So Please Give Unique Number` });
                        } else {
                ...
                case '23503': // Foreign key constraint violation
                    ...
                    if (error.detail) {
                        const match = error.detail.match(/Key \((.*?)\)=\((.*?)\)/);
                        if (match) {
                            errorDetails.push({ key: match[1], message: `Entered  ${match[1]}  Is Not Exist ${match[2]}.Please Provide Valid ${match[1]}` });
                        } else {
                            errorDetails.push({ key: 'unknown', message: error.detail });
                        }
                    }
                ...
                case '23502': // Not-null constraint violation
                    ...
                    if (error.detail) {
                        ...
                        if (match) {
                            errorDetails.push({ key: match[1], message: `Not-null constraint violation in column (${match[1]}).` });
                        } else {
                            errorDetails.push({ key: 'unknown', message: error.detail });
                        }
                    }
                ...
                case '23514': // Check constraint violation
                    ...
                    if (error.detail) {
                        ...
                        if (match) {
                            errorDetails.push({ key: 'check constraint', message: `Check constraint violation: ${error.detail}.` });
                        } else {
                            errorDetails.push({ key: 'unknown', message: error.detail });
                        }
                    }
                ...
                case '42703': // Undefined column error
                    ...
                    if (messageMatch) {
                        errorDetails.push({ key: messageMatch[1], message: `Undefined column: ${messageMatch[1]} in table: ${messageMatch[2]}.` });
                    } 
                    ...
                    if (error.detail) {
                        ...
                        if (match) {
                            errorDetails.push({ key: match[1], message: `Undefined column: ${match[1]}.` });
                        } else {
                            errorDetails.push({ key: 'unknown', message: error.detail });
                        }
                    }
                ...
                case '42P01': // Undefined table error
                    ...
                    errorDetails.push({ key: 'table', message: `Undefined table: ${error.message}.` });
                    break;
                case '23504': // Foreign key constraint violation due to deletion
                    ...
                    if (error.detail) {
                        const match = error.detail.match(/Key \((.*?)\)=\((.*?)\)/);
                        if (match) {
                            errorDetails.push({ key: match[1], message: `Foreign key constraint violation for key (${match[1]})=(${match[2]}).` });
                        } else {
                            errorDetails.push({ key: 'unknown', message: error.detail });
                        }
                    }
    ...
    export const handleQueryError = async (error: any) => {
        const { errorMessage, errorDetails, statusCode } = await checkErrorMessage(error as PostgresError);
