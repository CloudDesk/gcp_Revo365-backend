...
export const assertTransactionDateIsWithinAccountHistory = (
  ...
  if (openingDate && transactionDate < openingDate) {
    throw new FinanceValidationError(
      `Transaction date cannot be earlier than this account's opening balance date (${openingDate}).`
    );
  }
};
 ...
 */
export const rebuildBankCashAccountBalances = async (
  client: any,
  bankCashAccount: any,
  actor: string,
  epoch: number
  ...
  const transactionsResult = await client.query(
    `SELECT id, transactiondate, entryside, amount
     FROM bank_transactions
     WHERE bankcashaccountid = $1 AND postingstatus = 'posted'
     ORDER BY transactiondate ASC, posteddate ASC, id ASC`,
    [bankCashAccount.id]
  );
  ...
  for (const transaction of transactionsResult.rows) {
    ...
    if (transactionDate) {
      assertTransactionDateIsWithinAccountHistory(
        transactionDate,
        bankCashAccount.openingbalancedate
      );
    }
    currentBalance = calculateAvailableBalance(
      currentBalance,
      transaction.entryside,
      transaction.amount
    );
    balances.set(Number(transaction.id), currentBalance);
    await client.query(
      `UPDATE bank_transactions SET balanceafter = $1 WHERE id = $2`,
      [currentBalance, transaction.id]
    );
  }
  ...
  await client.query(
    `UPDATE bank_cash_accounts
     SET currentbalance = $1,
         ...
         modifieddate = $3
     WHERE id = $4`,
    [currentBalance, actor, epoch, bankCashAccount.id]
  );
