export type RentalSummaryInvoiceTemplateData = {
  companyName: string;
...
const renderImageOrLogoSlot = (data: RentalSummaryInvoiceTemplateData) => {
  if (data.logoUrl) {
...
const renderSignature = (data: RentalSummaryInvoiceTemplateData) => {
...
const renderTaxLabels = (data: RentalSummaryInvoiceTemplateData) =>
  data.taxMode === "igst"
    ? `<div>IGST ${escapeHtml(data.igstRate)}%</div>`
    : `<div>CGST ${escapeHtml(data.cgstRate)}%</div>
                <div>SGST ${escapeHtml(data.sgstRate)}%</div>`;

const renderTaxAmounts = (data: RentalSummaryInvoiceTemplateData) =>
  data.taxMode === "igst"
    ? `<div class="money-row">
