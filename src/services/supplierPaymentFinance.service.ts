...
const serializeOutstandingBill = (row: any) => {
...
const getExistingPayment = async (
  client: any,
...
export module supplierPaymentFinanceService {
  export const listSuppliers = async (request: any) => {
    ...
    for (const row of result.rows) {
      if (!isSupplierBillOpen(row)) continue;
      const state = getSupplierBillPaymentState(row);
      const supplierId = Number(row.supplierid);
      if (!Number.isSafeInteger(supplierId) || supplierId <= 0) continue;
      ...
      supplier.totaloutstanding = toMoney(
        supplier.totaloutstanding + state.outstandingAmount
      );
  ...
  export const listOutstandingBills = async (request: any) => {
  ...
  export const postPayment = async (request: any) => {
    ...
    if (requestedAllocations.length === 0) {
      throw new FinanceValidationError(
        "At least one supplier bill allocation is required."
      );
    }
    ...
    if (
      billIds.some((id) => !Number.isSafeInteger(id) || id <= 0) ||
      new Set(billIds).size !== billIds.length
    ) {
      throw new FinanceValidationError(
        "Bill allocations must contain unique valid bill IDs."
      );
    }
    ...
    if (allocationTotal !== amount) {
      throw new FinanceValidationError(
        "Payment amount must equal the total supplier bill allocation."
      );
    }
    ...
    try {
      ...
      if (account.status !== "active") {
        throw new FinanceValidationError(
          "Payments can only be posted from an active Bank/Cash account."
        );
      }
      ...
      if (billResult.rows.length !== billIds.length) {
        throw new FinanceValidationError(
          "One or more selected supplier bills were not found."
        );
      }
      const billById = new Map<number, any>(
        billResult.rows.map((row: any): [number, any] => [Number(row.id), row])
      );
      ...
      if (!Number.isSafeInteger(accountsPayableId)) {
        throw new FinanceValidationError(
          "Accounts Payable system ledger is unavailable.",
          409,
          "ACCOUNTS_PAYABLE_LEDGER_MISSING"
        );
      }
      ...
      if (hasTdsAllocations) {
        ...
        const tdsSectionIds = Array.from(
          new Set(
            preparedAllocations
      ...
      const sourceId = resolveAgainstDocumentSourceId(
        preparedAllocations.map(
          (allocation) =>
            allocation.bill.ponumber || allocation.bill.invoicenumber
        ),
        requestReference
      );
      const bankTransactionResult = await client.query(
      ...
      for (const allocation of preparedAllocations) {
        ...
        const invoiceStatus = resolveSupplierBillStatus(
          allocation.bill,
