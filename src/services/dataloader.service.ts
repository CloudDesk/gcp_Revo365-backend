...
import {
  ProductNumberFields,
  productArrayFields,
  productBooleanFields,
  productStringFields,
} from "../utils/Fields/productFields.js";
...
import {
  ...
  stocklocationArray,
  stockText,
} from "../utils/Fields/stockFields.js";
...
export module dataLoaderService {
  export const getDataLoaderData = async (request) => {
    try {
      ...
      await Promise.all(
        jsonresult.map(async (e: any, index: number) => {
          try {
            ...
            console.error(
              "Query Execution Error: IN getDataLoaderData promise",
              error
            );
      ...
      console.error(
        "Query Execution Error: IN getDataLoaderData common",
        error
      );
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
      return ErrorMessage;
      ...
      await Promise.all(
        jsonresult.map(async (e: any, index: number) => {
          try {
            ...
            console.error(
              "Query Execution Error: IN getDataLoaderDataStock promise",
              error
            );
            let ErrorMessage = await ErrorHandler.handleQueryError(error);
            return ErrorMessage;
