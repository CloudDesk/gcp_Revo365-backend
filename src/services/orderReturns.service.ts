...
export module orderReturnsService {
        ...
        if (input?.uniqueorderid && input?.productid) {
            const result = await query(
                `SELECT *
                 ...
                 ORDER BY id DESC
                 LIMIT 1`,
                [String(input.uniqueorderid), Number(input.productid)]
            );
            return result.rows[0] || null;
        }
    ...
    const resolveReturnReason = async (input: any) => {
        if (input?.reasonId) {
            const result = await query(
                `SELECT * FROM order_return_reasons WHERE id = $1 LIMIT 1`,
                [Number(input.reasonId)]
            );
            return result.rows[0] || null;
        }
        ...
        if (input?.reasonCode) {
            const result = await query(
                `SELECT * FROM order_return_reasons WHERE code = $1 LIMIT 1`,
    ...
    const assertReturnableLine = async (lineRow: any) => {
        if (!lineRow) {
        ...
        const activeResult = await query(
            `SELECT id, status
             ...
             ORDER BY id DESC
             LIMIT 1`,
            [Number(lineRow.id), OPEN_RETURN_STATUSES]
        );
    ...
    export const getReturnReasons = async (request?: any) => {
        const includeInactive = String(request?.query?.includeInactive || "").toLowerCase() === "true";
        const result = await query(
            `SELECT id, code, label, description, isactive, sortorder, created_at, updated_at
    ...
    export const upsertReturnReason = async (request: any) => {
        ...
        const result = id
            ...
            : await query(
                `INSERT INTO order_return_reasons (code, label, description, sortorder, isactive)
                 VALUES ($1, $2, $3, $4, $5)
                 RETURNING *`,
                params
            );
    ...
    export const createReturnRequest = async (request: any) => {
        const body = request?.body || {};
    ...
    export const getReturnRequests = async (request: any) => {
    ...
    const updateReturnRequestStatus = async (
        requestId: number,
        nextStatus: string,
        actorId: number | null,
        adminComment?: string | null,
        extraAssignments?: Record<string, any>
    ) => {
        const existing = await query(
            `SELECT * FROM orderline_returns WHERE id = $1 LIMIT 1`,
            [requestId]
        );
        ...
        const assignments: string[] = [`status = $1`, `updated_at = NOW()`];
    ...
    export const approveReturnRequest = async (request: any) => {
        const actorId = resolveSessionUserId(request);
    ...
    export const rejectReturnRequest = async (request: any) => {
        const actorId = resolveSessionUserId(request);
        const requestId = Number(request?.body?.id);
        const adminComment = request?.body?.adminComment ? String(request.body.adminComment).trim() : null;
        if (!adminComment) {
    ...
    export const receiveReturnRequest = async (request: any) => {
        const actorId = resolveSessionUserId(request);
        const requestId = Number(request?.body?.id);
        const adminComment = request?.body?.adminComment ? String(request.body.adminComment).trim() : null;
    ...
    export const finalizeReturnRequest = async (request: any) => {
        const actorId = resolveSessionUserId(request);
        const requestId = Number(request?.body?.id);
        const adminComment = request?.body?.adminComment ? String(request.body.adminComment).trim() : null;
        ...
        await ordersService.handleReturnedOrderLines(
            [lineRow],
            new Map<number, string>([[Number(lineRow.id), normalizeStatus(previousLineRow.orderstatus)]])
        );
        await ordersService.syncOrderHeadersFromOrderLines([lineRow.uniqueorderid]);

        const updatedRequest = await updateReturnRequestStatus(
            requestId,
            "finalized",
            actorId,
            adminComment,
