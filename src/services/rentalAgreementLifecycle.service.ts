...
const resolveAgreementClosureStatus = async (executor: any, agreementId: number) => {
...
export type UpdateAgreementAssetLifecycleStateOptions = {
  executor: any;
  ...
  deactivateAsset?: boolean;
};

export const updateAgreementAssetLifecycleState = async ({
  executor,
  agreementId,
  orderlineId,
  assetNumber = null,
...
}: UpdateAgreementAssetLifecycleStateOptions) => {
  ...
  const closureStatus = await resolveAgreementClosureStatus(
    executor,
    Number(agreementId)
  );
  ...
  if (closureStatus) {
    const agreementUpdateResult =
      closureStatus === AGREEMENT_STATUS_COMPLETED
