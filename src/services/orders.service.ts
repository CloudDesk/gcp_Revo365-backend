  ...
  export const getUserOrderData = async (request: any) => {
    try {
      ...
      if (offset != null && recordCount != null) {
        queryText += ` OFFSET $${queryParams.length + 1} LIMIT $${
  ...
  export const getOrderlineDynamic = async (request) => {
    try {
      ...
      const invoiceMap = new Map(
        invoiceurldata.rows.map((row) => [row.orderid, row.invoiceurl]),
      );
      ...
      if (data.rows.length > 0) {
        const productIds = data.rows
          .map((row) => row.productid)
          .filter((id) => id != null);
        if (productIds.length > 0) {
          const productImageParams = productIds
            .map((_, idx) => `$${idx + 1}`)
            .join(",");
...
${whereClause} ${orderByClause}`;
      ...
      let datatypeCheckResult = await dataTypeCheck(result);
      return {
        data: datatypeCheckResult,
        total: Array.isArray(datatypeCheckResult)
          ? datatypeCheckResult.length
          : 0,
      };
    } catch (error) {
  ...
  export const upsertOrder = async (orderData: any) => {
    try {
      ...
      if (newStatus === "cancelled") {
        ...
        if (orderType === "rental") {
          await stockRevoService.releaseReservedRentalStockForOrder(
            updatedRow.orderid,
          );
        }

        const userid = updatedRow.userid;
        const getuser = await query(`SELECT * FROM users WHERE id = $1`, [
          userid,
        ]);
        ...
        console.log(
          `[upsertOrder] Released orderedquantity for ${newStatus} order ${updatedRow.orderid}`,
        );
      }
      return result;
    } catch (error) {
  ...
  export const updateorderlineitem = async (orderlineData: any) => {
    try {
      console.log(
        "Inside updateorderlineitem function with data:",
        orderlineData,
      );
      let querydata: string;
      let params: any[];
      ...
      if (lineStatus === "cancelled") {
        ...
        if (orderType === "rental") {
          await stockRevoService.releaseReservedRentalStockForOrderline(
            lineRow.orderlinenumber,
          );
        }
        ...
        if (lineType === "Orders") {
          ...
          console.log(
            `[updateorderlineitem] Released orderedquantity for ${lineStatus} orderline ${lineRow.orderlinenumber}`,
          );
        }
      }
      return result;
  ...
  const getBillingChainKey = (row: any) =>
    Number(row.parentorderlineid ?? row.id);

  export const getInvoiceGeneratedData = async (request: any) => {
    try {
      console.log(
        "Inside getInvoiceGeneratedData function with request:",
        request.params,
        request.query,
      );
      const orderId = request.params.uniqueorderid;
      const requestedOrderlineIds = parseOrderlineIds(
        request.query?.orderlineids,
      );
      ...
      return {
        ...
        generatedmonthscount: Math.max(
          ...rows.map((r: any) => r.generatedmonthscount ?? 0),
        ),
  ...
  export const updateInvoiceGeneratedData = async (request: any) => {
    try {
      ...
      const requestedOrderlineIds = parseOrderlineIds(
        request.body?.orderlineids,
      );
      console.log(
        "Unique Order ID:",
        uniqueorderid,
        "Requested orderline ids:",
        requestedOrderlineIds,
      );
      ...
      if (!rows.length) {
        return {
          success: false,
          message: "No active billing orderlines found",
        };
      }
      ...
      if (billingconflictchains.length > 0) {
        return {
          success: false,
          message:
            "Multiple active billing lines exist in the same contract chain. Reconcile the billing chain before generating rental invoices.",
          billingconflictchains,
        };
      }
      ...
      if (!stillActive.length) {
        return {
          success: false,
          message: "No active rental products to update",
        };
      }
   ...
   RETURNING id, rentalfor, generatedmonthscount, invoicegenerated, lastgeneratedinvoicedate`,
      ...
      console.error(
        "Query Execution Error: IN updateInvoiceGeneratedData",
        error,
      );
      let ErrorMessage = await ErrorHandler.handleQueryError(error);
      return ErrorMessage;
  ...
  export const upsertOrderrfid = async (orderData: any) => {
    try {
      ...
      let updateStock: any =
        await stockRevoService.upsertStockRevoDatarfid(orderData);

      if (
        updateStock.command === "UPDATE" ||
        updateStock.command === "INSERT"
      ) {
        ...
        if (!ordername && orderData[0]?.orderlinenumber) {
          console.log(
            "DEBUG: ordername not in request, querying database with orderlinenumber:",
        ...
        console.log(
          "DEBUG: Final ordername:",
          ordername,
          "isRental:",
          isRental,
        );

        let updateQuantity = await stockRevoService.updateQuantity(
          pucArray,
        ...
        const ordersToUpdate = updateStock.result.rows.filter(
          (e) => e.orderlinenumber,
        ); // Only consider rows with an orderid
    ...
    try {
      ...
      for (const item of orderData) {
        if (rfidMap.has(item.rfid)) {
          return {
            error:
              "Duplicate RFID detected: Same RFID has been scanned multiple times. Please scan a different RFID to proceed.",
            errorDetails: [],
            statusCode: 401,
          };
        }
      ...
      const isRentalValidation =
        normalizeComparableText(ordername) === "rental";
      const validationParams: any[] = [];
      const validationWhere = orderData
        .map((item) => {
      ...
      if (validationResult.rows.length !== orderData.length) {
        const foundRfids = new Set(
          validationResult.rows.map((row) => row.rfid),
        );
        const invalidRfids = orderData.filter(
          (item) => !foundRfids.has(item.rfid),
        );
        return {
      ...
      if (updateStock.error) {
        ...
        updateStock &&
        (updateStock.command === "UPDATE" || updateStock.command === "INSERT")
      ) {
        const pucArray: string[] = Array.from(
          new Set(updateStock.result.rows.map((row) => row.puc)),
        );
        ...
        if (ordername) {
          ...
          console.log(
            "DEBUG: No ordername found, checking product_revo for rental status",
          );
          ...
          if (productQuery.rows.length > 0) {
            ...
            console.log(
              "DEBUG: Product ecompublish status:",
              productQuery.rows[0].ecompublish,
              "isRental:",
              isRental,
            );
          }
  ...
  export const bulkInsertOrder = async (
    transactionData: any,
    orderData: any,
  ) => {
    try {
      ...
      const { merchantTransactionId, userId, cgst, sgst, storelocation } =
        transactionData;
      ...
      const availableQuantities = quantityResult.rows.reduce(
        (acc: any, row: any) => {
          acc[row.productid] = {
      ...
      orderData.forEach((item: any) => {
        ...
        const isRentalOrder =
          String(item.invoicefor ?? "")
            .trim()
            .toLowerCase() === "product rental";
        ...
        if (requestedQuantity <= available) {
          ...
          if (isRentalOrder) {
            throw new Error(
              `Insufficient rental stock available for product ${item.productid}.`,
            );
          }
          if (available > 0) {
            // Add available quantity to orders
      ...
      if (ordersToInsert.length > 0) {
        ...
        try {
          ...
          if (orderResult.command === "INSERT") {
            ...
            const orderlineResult = await bulkInsertOrderlines(ordersToInsert);
            console.log(
              "Order lines inserted from orders:",
              orderlineResult.rows,
            );
            console.log("Empty After inserting order lines");
            // Add orders rows to combined result
            combinedResult.rows = [...combinedResult.rows, ...orderResult.rows];
      ...
      if (thirdPartyOrdersToInsert.length > 0) {
        ...
        let thirdPartyOrderQuantity = thirdPartyOrdersToInsert.reduce(
          (acc: number, e: any) => {
            return acc + e.quantity;
          },
          0,
        );
        let thirdPartyOrderAmount = thirdPartyOrdersToInsert.reduce(
          (acc: number, e: any) => {
            return acc + e.productamount * e.quantity;
          },
          0,
        );
        let thirdPartyProductIds = thirdPartyOrdersToInsert.map(
          (e: any) => e.productid,
        );

        console.log(
          "Order quantity for thirdpartyorders:",
          thirdPartyOrderQuantity,
        );
        console.log(
          "Order amount for thirdpartyorders:",
        ...
        try {
          const thirdPartyResult = await query(
            insertThirdPartyQuery,
            insertThirdPartyValues,
          );
          console.log("Third-party order result:", thirdPartyResult.rows);
          if (thirdPartyResult.command === "INSERT") {
            const orderid = thirdPartyResult.rows[0].id;
            ...
            const orderlineResult = await bulkInsertOrderlines(
              thirdPartyOrdersToInsert,
            );
            console.log(
              "Order lines inserted from third party:",
              orderlineResult.rows,
            );
            console.log("Empty After inserting third-party order lines");
            // Add thirdpartyorders rows to combined result
            combinedResult.rows = [
              ...combinedResult.rows,
              ...thirdPartyResult.rows,
            ];
          }
        } catch (error) {
          console.error(
            "Query Execution Error: BulkinsertThirdPartyOrder result",
            error,
          );
          let ErrorMessage = await ErrorHandler.handleQueryError(error);
          return ErrorMessage;
  ...
  export const bulkInsertOrderlines = async (orderData: any[]) => {
    try {
      console.log(
        "Inside update bulkInsertOrderlines with orderData:",
        JSON.stringify(orderData, null, 2),
      );
      const fields = Object.keys(orderData[0]);
      const fieldNames = fields.join(", ");
      const baseQuery = `INSERT INTO orderline (${fieldNames}) VALUES `;
      const valuesClause = orderData
        .map((order, index) => {
          const valuePlaceholders = fields.map(
            (_, fieldIndex) => `$${index * fields.length + fieldIndex + 1}`,
          );
    ...
    try {
      ...
      if (updateValuesArray.length > 0) {
        ...
        if (updatedOrderResult.command === "UPDATE") {
          ...
          const updatedOrderLineData = await ordersService.updateOrderStatus(
            orderlinedata,
            emailid,
            paymentfailed,
            false,
          ...
          if (!paymentfailed) {
            const rentalOrders = updatedOrderResult.rows.filter(
              (row: any) => row.ordername === "rental",
            );
            if (rentalOrders.length > 0) {
              console.log(
                `Found ${rentalOrders.length} rental orders. Allocating stock.`,
              );
          ...
          console.log(
            "Updated Order Line Data from orders:",
            updatedOrderLineData,
          );
          console.log("cdc line data");
          return { data: updatedOrderResult.rows, status: "success" };
        } else {
          return {
            data: `Orders Not Updated Please contact admin`,
            status: "failure",
          };
        }
      }
    ...
    try {
      ...
      const updateQuery = isThirdParty
        ...
        : `
            ...
            `;
      console.log(
        "Inside updateOrderStatus with data: Update Query:",
        updateQuery,
      );
      const proceessId = isThirdParty
...
Thank You!`,
  ...
  export const getOrderDataForMerchantid = async (merchantiddata: any) => {
    try {
      ...
      const productIdOrderlineResult = await query(productIdOrderlineQuery, [
        uniqueorderid,
      ]);
  ...
  export const deleteFailedOrder = async (merchantid) => {
    ...
    `;
      ...
      if (orderIdResult.rows.length === 0) {
        return {
          status: 200,
          message: "Merchant Id Payment is successful or no pending orders",
        };
      }
    ...
    `;
          ...
          const res = await query(updateLockQtyQuery, [
            product.quantity,
            product.productid,
          ]);
