 ...
 export const updateorderlineitem = async (orderlineData: any) => {
    try {
        ...
        if (lineRow?.uniqueorderid && addressSnapshotFields.length > 0) {
            ...
            await query(
                `UPDATE orders SET ${assignments} WHERE orderid = $${orderIdParameter}`,
                [...addressValues, lineRow.uniqueorderid]
            );
            await query(
                `UPDATE orderline SET ${assignments} WHERE uniqueorderid = $${orderIdParameter}`,
                [...addressValues, lineRow.uniqueorderid]
            );
        }
