...
export module productService {
  ...
  export const getSimilarProducts = async (
    ...
    try {
      ...
      if (result.rows.length <= 1) {
        ...
        const resultLatest: QueryResult = await query(
          queryTextLatest,
          queryParamsLatest
        );
  ...
  export const getArcheivedProducts = async (
    pageNumber: number,
    recordCount: number,
    request: any
  ) => {
    ...
    try {
      ...
      if (upsertProductData.id) {
        existingProductData = await query(
          `SELECT * FROM products where id=${upsertProductData.id}`,
          {}
        );
      }
      let data: any = {};
      ...
      let params: any[] = [];
      if (id) {
        let result = await getStockData(stockFields, stockValues);
        if (result) {
      ...
      let updatestockdataset = await updatestockQuantity(
        result.rows[0].puc,
        oldpucvalue
      );
  ...
  export const upsertProductwithFile = async (request: any) => {
    try {
  ...
  export const rearrangeImage = async (request) => {
    try {
      ...
      let getData = await query(
        `select large,medium,small from products where id =${productid}`,
        {}
      );
