...
export module locationHistoryService {
    export const getLocationHistoryData = async (request: any) => {
        try {
    ...
    export const upsertLocationHistory = async (locationdata : any) => {
        try {
            ...
            if (id) {
                ...
                querydata = `INSERT INTO locationhistory (${fieldNames.join(
                    ", "
                )}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
