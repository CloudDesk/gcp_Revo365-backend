...
export module productrevoService {
  ...
  export const upsertProductrevo = async (productrevoData: any) => {
    ...
    try {
      ...
      if (id) {
        ...
        const querydata = `INSERT INTO product_revo (${fieldNames.join(
          ", "
        )}) VALUES (${fieldNames
          .map((_, index) => `$${index + 1}`)
          .join(", ")}) RETURNING *`;
  ...
  const validateComputerProductConfiguration = async (client: any, productData: any) => {
    ...
    if (subcategory !== "computer") return;
    ...
    for (const component of components) {
      ...
      if (!roleCode) throw new Error("Every BOM component requires a Component Role.");
    ...
    if (buildType === "custom_assembled") {
      const requiredRoles = await getRequiredComputerRoleCodes(
        client,
        packageType,
        buildType
      );
      const configuredRoles = new Set(
        components
          .filter((component: any) => Number(component?.component_product_id) > 0)
          .map((component: any) => String(component.component_role_code))
      );
      const missingRoles = requiredRoles.filter((role) => !configuredRoles.has(role));
      if (missingRoles.length) {
        throw new Error(
          `Select products for all required custom-build components: ${missingRoles
    ...
    if (ASSEMBLY_FULFILLMENT_TYPES.has(fulfillmentType)) {
      const componentProductIds = components
        .map((component: any) => Number(component?.component_product_id))
  ...
  const getRequiredComputerRoleCodes = async (
    client: any,
    packageType: string,
    buildType: string
  ): Promise<string[]> => {
    const result = await client.query(
      `SELECT value.code, value.metadata_json
  ...
  export const getProductComponentOptions = async (request: any) => {
    try {
      ...
      if (search) {
        params.push(`%${search}%`);
        conditions.push(
          `(productname ILIKE $${params.length}
  ...
  export const getProductBom = async (productId: number) => {
    try {
      const result = await query(
        `SELECT
           bom.*,
