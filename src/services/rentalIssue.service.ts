...
const CONTRACT_CLOSE_REASON_DAMAGED_NON_RETURNABLE =
...
const buildLifecycleRemarks = (
  ...
  if (normalizedReason && normalizedRemarks) {
    if (
      normalizeComparableText(normalizedReason) ===
...
const ensureStockBelongsToContract = (
  stockRow: any,
  context: any,
  assetNumber: string
) => {
  if (!stockRow) {
    throw new Error(
      "Unable to find the current stock record linked to this rental asset."
    );
  }

  const linkedOrderLineNumber = normalizeText(
  ...
  if (
    ...
    throw new Error(
      "The resolved stock record does not match the asset linked to this ticket."
    );
  }
};

const findLatestRentalInvoiceId = async (executor: any, uniqueOrderId: string | null) => {
...
const ensurePenaltyInvoiceEligibility = (
  context: any,
  requestedPenaltyType?: string | null
) => {
  ...
  const ticketActionStatus = normalizeComparableText(
    context?.ticket?.rentalactionstatus
  );
  const ticketDamageAssessment = normalizeComparableText(
    context?.ticket?.damageassessment
  );

  const inferredPenaltyType =
    ticketActionType === TICKET_LOST_ACTION_TYPE ||
  ...
  if (!inferredPenaltyType) {
    throw new Error(
      "Penalty invoices are allowed only for lost or non-returnable damaged rental assets."
    );
  }

  const normalizedRequestedPenaltyType = normalizeComparableText(
    requestedPenaltyType
  );

  if (
    normalizedRequestedPenaltyType &&
    normalizedRequestedPenaltyType !== inferredPenaltyType
  ) {
    throw new Error(
      "Penalty type does not match the current rental asset lifecycle state."
    );
  }
...
export type ProcessRentalLostOptions = {
  executor: any;
  context: any;
  ticketId: number;
...
export const processRentalLost = async ({
  executor,
  context,
  ticketId,
  oldAssetNumber,
...
export type ProcessRentalDamageAssessmentOptions = {
  executor: any;
  context: any;
  ticketId: number;
  oldAssetNumber: string;
...
export const processRentalDamageAssessment = async ({
  executor,
  context,
  ticketId,
  oldAssetNumber,
  ...
  remarks = null,
  createdBy = null,
}: ProcessRentalDamageAssessmentOptions) => {
  const linkedOrderline = context?.linkedorderline;
  if (!linkedOrderline) {
    throw new Error("Unable to resolve the active rental contract for this ticket.");
  }
  ...
  if (
    normalizedAssessment !== RETURNABLE_DAMAGE_ASSESSMENT &&
...
export type LinkRentalPenaltyInvoiceOptions = {
  executor: any;
  context: any;
  ticketId: number;
  penaltyInvoiceId: number;
...
export const linkRentalPenaltyInvoice = async ({
  executor,
  context,
  ticketId,
  penaltyInvoiceId,
  ...
  createdBy = null,
  penaltyType = null,
}: LinkRentalPenaltyInvoiceOptions) => {
  const linkedOrderline = context?.linkedorderline;
  if (!linkedOrderline) {
    throw new Error("Unable to resolve the active rental contract for this ticket.");
  }
  ...
  const normalizedPenaltyType = ensurePenaltyInvoiceEligibility(
    context,
    penaltyType
  );
  ...
  if (
    normalizeComparableText(penaltyInvoiceRecord.invoicefor) !== "penalty"
  ) {
  ...
  const agreementId = resolveAgreementId(context);
  const sourceInvoiceId = await findLatestRentalInvoiceId(
    executor,
