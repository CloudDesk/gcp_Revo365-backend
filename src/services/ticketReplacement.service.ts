...
import {
  linkRentalPenaltyInvoice,
  processRentalDamageAssessment,
  processRentalLost,
...
const toOptionalPositiveBigInt = (value: any) => {
...
const parseInvoiceDate = (invoiceRow: any) => {
  ...
  if (Number.isFinite(invoiceDateValue) && invoiceDateValue > 0) {
    const normalizedInvoiceDate =
      String(Math.trunc(invoiceDateValue)).length <= 10
  ...
  if (Number.isFinite(createdDateValue) && createdDateValue > 0) {
    const normalizedCreatedDate =
      String(Math.trunc(createdDateValue)).length <= 10
...
const getBillingMonthIndex = (startDate: Date, targetDate: Date) => {
  let monthsDiff =
...
const buildReplacementContext = async (ticketId: number) => {
  ...
  if (normalizedTicketType !== REPAIR_RENTAL_TICKET_TYPE) {
    throw new Error(
      "Rental action flow is available only for Repair Rental tickets."
    );
  }

  const linkedOrderline =
    ticketRow.resolved_orderlineid == null
      ? null
      : {
          ...
          generatedmonthscount: Number(
            ticketRow.resolved_generatedmonthscount ?? 0
          ),
          invoicegenerated: ticketRow.resolved_invoicegenerated,
          lastgeneratedinvoicedate:
            ticketRow.resolved_lastgeneratedinvoicedate,
  ...
  const orderlineGeneratedMonths = Number(
    linkedOrderline?.generatedmonthscount ?? 0
  );
  const billedCycleIndices = new Set<number>();
  ...
  if (
    ...
    invoiceResult.rows.forEach((invoiceRow: any) => {
  ...
  const monthsalreadybilled = Math.max(
    orderlineGeneratedMonths,
    invoicedMonths
  );
  const remainingmonths = Math.max(
    Number(linkedOrderline?.rentalfor ?? 0) - monthsalreadybilled,
    0
  );

  const resolvedAssetNumber = normalizeText(
    linkedOrderline?.assetnumber ?? ticketRow.assetnumber
  );
    ...
    invoiceprogress: {
      ...
      lastinvoicedate:
        invoiceResult.rows.length > 0
          ...
          : null,
      iscurrentcycleinvoiced:
        rentalStartDate && !Number.isNaN(rentalStartDate.getTime())
          ? billedCycleIndices.has(
              getBillingMonthIndex(rentalStartDate, new Date())
            )
          : false,
    },
...
const getActiveReplacementRecord = async (
  executor: any,
  context: any,
  ticketId: number
) => {
  ...
  if (activeReplacementId) {
    result = await executor.query(
      `SELECT * FROM rental_replacement_history WHERE id = $1 LIMIT 1`,
      [activeReplacementId]
    );
  } else {
    result = await executor.query(
...
const isStopRentalTicket = (context: any) =>
  normalizeComparableText(context?.ticket?.rentalactiontype) ===
  STOP_RENTAL_ACTION_TYPE;

const createStopRentalHistoryRecord = async (
  executor: any,
  context: any,
  ticketId: number,
...
const getReplacementCandidateStock = async (
  assetOrRfid: string,
...
export module ticketReplacementService {
  export const getRentalReplacementContext = async (request: any) => {
    try {
      const ticketId = toPositiveInteger(request.params.id);
      return await buildReplacementContext(ticketId);
    } catch (error) {
      console.error(
        "Query Execution Error: IN getRentalReplacementContext",
        error
      );
      throw error;
    }
  ...
  export const getRentalReplacementHistory = async (request: any) => {
    try {
      const ticketId = toPositiveInteger(request.params.id);
      ...
      console.error(
        "Query Execution Error: IN getRentalReplacementHistory",
        error
      );
      throw error;
    }
  ...
  export const initiateRentalReplacement = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;

    try {
      const ticketId = toPositiveInteger(request.params.id);
      const replacementtype = normalizeComparableText(
        request.body?.replacementtype
      );
      const remarks = normalizeText(request.body?.remarks);

      if (!ALLOWED_REPLACEMENT_TYPES.has(replacementtype)) {
        throw new Error(
          "Replacement type must be technical_replacement or commercial_replacement."
        );
      }
      ...
      if (!context.linkedorderline) {
        throw new Error(
          "Unable to resolve the active rental contract for this ticket."
        );
      }

      const latestReplacement = await getActiveReplacementRecord(
        client,
        context,
        ticketId
      );
      ...
      if (
        latestReplacement &&
        !FINAL_REPLACEMENT_STATUSES.has(
          normalizeComparableText(latestReplacement.replacementstatus)
        )
      ) {
        throw new Error(
          "An active replacement flow already exists for this ticket."
        );
      }
      ...
      const historyInsertResult = await client.query(
        ...
        [
          ...
          INITIATED_REPLACEMENT_STATUS,
          normalizeText(
            context.linkedorderline.assetnumber ?? context.ticket.assetnumber
          ),
          context.linkedorderline.productid,
          context.monthsalreadybilled,
          context.remainingmonths,
      ...
      console.error(
        "Query Execution Error: IN initiateRentalReplacement",
        error
      );
      throw new Error(
  ...
  export const receiveOldAsset = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      ...
      const activeReplacement = await getActiveReplacementRecord(
        client,
        context,
        ticketId
      );
      ...
      if (!activeReplacement && !isStopRentalRequest) {
        throw new Error(
          "Initiate rental replacement before receiving the old asset."
        );
      }

      const replacementStatus = normalizeComparableText(
        activeReplacement?.replacementstatus
      );

      if (activeReplacement && FINAL_REPLACEMENT_STATUSES.has(replacementStatus)) {
        throw new Error(
          "This replacement flow is already closed and cannot receive the old asset."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Old asset can be received only after replacement initiation and before further replacement actions."
        );
      }

      const expectedOldAssetNumber = normalizeText(
        activeReplacement?.oldassetnumber ??
          context.linkedorderline.assetnumber ??
          context.ticket.assetnumber
      );

      if (
        expectedOldAssetNumber &&
        normalizeComparableText(oldassetnumber) !==
          normalizeComparableText(expectedOldAssetNumber)
      ) {
        throw new Error(
          `Old asset number mismatch. Expected ${expectedOldAssetNumber}.`
        );
      }
      ...
      if (!stockToReceive) {
        throw new Error(
          "Unable to find the currently assigned stock record for this rental asset."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "The resolved stock record does not belong to the linked rental order line."
        );
      }
      ...
      if (!ensuredActiveReplacement) {
        ensuredActiveReplacement = await createStopRentalHistoryRecord(
          client,
          context,
          ticketId,
          oldassetnumber,
  ...
  export const assignTechnicalReplacement = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    let agreementSyncResult: any = null;
    ...
    try {
      ...
      if (!activeReplacement) {
        throw new Error(
          "Initiate rental replacement before assigning the replacement asset."
        );
      }

      const replacementType = normalizeComparableText(
        activeReplacement.replacementtype ?? context.ticket.replacementtype
      );
      const replacementStatus = normalizeComparableText(
        activeReplacement.replacementstatus
      );
      ...
      if (replacementType !== "technical_replacement") {
        throw new Error(
          "This endpoint supports only technical replacement assignments."
        );
      }

      if (FINAL_REPLACEMENT_STATUSES.has(replacementStatus)) {
        throw new Error(
          "This replacement flow is already closed and cannot assign a replacement asset."
        );
      }

      const oldAssetNumber = normalizeText(
        activeReplacement.oldassetnumber ??
          context.linkedorderline.assetnumber ??
          context.ticket.assetnumber
      );
      const ticketReplacementStatus = normalizeComparableText(
        context.ticket.replacementstatus
      );
      const historyAssignedAssetNumber = normalizeText(
        activeReplacement.newassetnumber
      );
      const ticketAssignedAssetNumber =
        ticketReplacementStatus === ASSIGNED_REPLACEMENT_STATUS
          ? normalizeText(context.ticket.assetnumber)
          : null;
      const orderlineAssignedAssetNumber =
        context.linkedorderline.assetnumber &&
      ...
      if (
        ...
        if (assignedAssetCandidates.length > 1) {
          throw new Error(
            "Replacement asset is already assigned for this ticket, but the ticket and order line are out of sync. Please reconcile the current assigned asset before retrying."
          );
        }

        throw new Error(
          "Replacement asset has already been assigned for this ticket."
        );
      }

      if (replacementStatus !== OLD_ASSET_RECEIVED_STATUS) {
        throw new Error(
          "Receive the old asset before assigning the replacement asset."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Replacement asset number must be different from the old asset number."
        );
      }
      ...
      if (!candidateStock) {
        throw new Error(
          "No stock record found for the provided replacement asset number or RFID."
        );
      }

      if (
        normalizeComparableText(candidateStock.stocktype) !== "rental_product"
      ) {
        throw new Error(
          "Only rental product stock can be assigned as a technical replacement."
        );
      }

      if (candidateStock.ecompublish === true) {
        throw new Error(
          "Replacement stock must come from rental inventory only."
        );
      }

      if (
        normalizeComparableText(candidateStock.stockstatus) !== "available"
      ) {
        throw new Error(
          "Replacement stock must be in Available status before assignment."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Replacement stock is already linked to another order line."
        );
      }
      ...
      if (agreementId != null) {
        ...
        } catch (agreementPdfError: any) {
          console.error(
            "Technical replacement agreement PDF refresh failed",
            agreementPdfError
          );
          agreementWarning =
      ...
      console.error(
        "Query Execution Error: IN assignTechnicalReplacement",
        error
      );
      throw new Error(
        error?.message || "Failed to assign the technical replacement asset."
      );
    } finally {
      client.release();
    }
  ...
  const buildNewOrderLineNumber = (baseOrderLineNumber: any) => {
    const base = normalizeText(baseOrderLineNumber) ?? "";
    if (!base) return `ordline-${Date.now()}`;
  ...
  const toEpochSeconds = (dateValue: Date) => {
    return Math.trunc(dateValue.getTime() / 1000);
  };

  const addMonthsUtc = (dateValue: Date, months: number) => {
    const result = new Date(dateValue.getTime());
    result.setUTCMonth(result.getUTCMonth() + months);
    return result;
  };

  const getCommercialCutoverDate = (
    billingMode: string,
    ...
    let monthsDiff =
      (effectiveDate.getUTCFullYear() - anchorDate.getUTCFullYear()) * 12 +
  ...
  const formatTicketProductCategory = (value: any) => {
  ...
  const getProductSummary = async (executor: any, productId: number) => {
  ...
  const findStockForContract = async (
    executor: any,
    options: {
      assetnumber?: any;
      linkedorderlinenumber?: any;
      ticketorderlinenumber?: any;
      holdticketid?: any;
    }
  ) => {
    const result = await executor.query(
  ...
  const restoreOldAssetToContract = async (
    executor: any,
    context: any,
    activeReplacement: any,
    ticketId: number
  ) => {
    const oldAssetNumber = normalizeText(
      activeReplacement.oldassetnumber ??
  ...
  const stopActiveRentalContract = async (
    executor: any,
    context: any,
    ticketId: number,
    stopEpoch: number,
    ...
    if (options?.holdAsset !== false) {
      const contractAssetNumber = normalizeText(
        context.linkedorderline.assetnumber ?? context.ticket.assetnumber
      );
  ...
  export const assignCommercialReplacement = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    let agreementSyncResult: any = null;
    ...
    try {
      ...
      const candidateStock = await getReplacementCandidateStock(
        newassetnumber,
        Number(newproductid)
      );
      if (!candidateStock) {
      ...
      const commercialCutoverDate = getCommercialCutoverDate(
        billingmode,
        effectivefrom,
        context.linkedorderline.rentstartdate
      );
      ...
      const persistedOrderlinenumber = normalizeText(
        insertedNewOrderline.rows[0].orderlinenumber
      );
      ...
      if (agreementId != null) {
        try {
          ...
          console.error(
            "Commercial replacement agreement PDF refresh failed",
            agreementPdfError
          );
          agreementWarning =
            agreementPdfError?.message ||
  ...
  export const rejectReplacement = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      ...
      let stoppedOrderline: any = null;
      let restoredAssetNumber: string | null = null;
      ...
      if (rejectionaction === "continue_old_asset" || rejectionaction === "close_ticket") {
        if (replacementStatus === OLD_ASSET_RECEIVED_STATUS) {
          const restoreResult = await restoreOldAssetToContract(
            client,
            context,
            activeReplacement,
            ticketId
      ...
      const message =
        rejectionaction === "continue_old_asset"
  ...
  export const returnRentalAsset = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      ...
      if (context.ticket.stoprental === true) {
        throw new Error(
          "This ticket already stopped the rental contract. Process post-stop returns in the next lifecycle slice."
        );
      }
      ...
      if (
        activeReplacement &&
        !FINAL_REPLACEMENT_STATUSES.has(
          normalizeComparableText(activeReplacement.replacementstatus)
        )
      ) {
        throw new Error(
          "An active replacement flow exists for this ticket. Finish or reject the replacement before returning the asset."
        );
      }
      ...
      const ticketRentalActionStatus = normalizeComparableText(
        context.ticket.rentalactionstatus
      );

      if (
        context.linkedorderline.isactivebillingline === false ||
  ...
  export const markRentalAssetLost = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      ...
      if (normalizeComparableText(context.ticket?.rentalactiontype) !== "lost") {
        throw new Error(
          "This ticket is not marked as a Lost rental request."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "An active replacement flow exists for this ticket. Finish or reject the replacement before marking the asset as lost."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "This rental asset already has a terminal lifecycle status and cannot be marked lost again."
        );
      }
  ...
  export const assessRentalDamage = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      const ticketId = toPositiveInteger(request.params.id);
      const oldassetnumber = normalizeText(request.body?.oldassetnumber);
      const damageassessment = normalizeComparableText(
        request.body?.damageassessment
      );
      ...
      if (normalizeComparableText(context.ticket?.rentalactiontype) !== "damaged") {
        throw new Error(
          "This ticket is not marked as a Damaged rental request."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "An active replacement flow exists for this ticket. Finish or reject the replacement before assessing damage."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "This rental asset already has a terminal lifecycle status and cannot be assessed again."
        );
      }
      ...
      return {
        message:
          damageassessment === "non_returnable"
      ...
      throw new Error(
        error?.message || "Failed to assess rental asset damage."
      );
    } finally {
      client.release();
    }
  ...
  export const linkPenaltyInvoice = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
  ...
  export const renewRentalContract = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
    ...
    try {
      ...
      const approvedrenewaldate =
        request.body?.approvedrenewaldate ?? requestedrenewaldate;
      const remarks = normalizeText(request.body?.remarks);
      ...
      if (
        ...
        throw new Error(
          "This rental asset already has a terminal lifecycle status and cannot be renewed."
        );
      }
  ...
  export const stopRental = async (request: any) => {
    const client = await pool.connect();
    let transactionStarted = false;
