    ...
    export const paymentConfirmation = async (request: any, reply: any) => {
        try {
            ...
            if (response.data.code && response.data.code == 'PAYMENT_SUCCESS') {
                ...
                let result: any = await insertTransactionData(transactionDataset, insersertdordderdatawithprocessing)
                if (result.orderdata && result.orderdata.length > 0 && result.transactionData && result.transactionData.length > 0) {
                    ...
                    if (productupdateorderqty.length > 0) {
                        ...
                        const updatedOrderQuantity: any = await productrevoService.updateOrderedQuantityarray(updateproductorderquantiydata)
                ...
                let result: any = await insertTransactionData(transactionDataset, insersertdordderdatawithprocessing, true)
...
console.log(cloudflaretoken ,'Cloudflare Token is');
    ...
    export const insertTransactionData = async (transactionData: any, insersertdordderdatawithprocessing: any, paymentfailed = false) => {
        try {
            const {
