...
export module picklistConfigService {
  export const getDefinitions = async (request: any) => {
    try {
  ...
  export const upsertDefinition = async (payload: any) => {
    try {
      ...
      if (id) {
        const existingDefinition = await query(
          `SELECT is_system_controlled FROM picklist_definitions WHERE id = $1`,
          [id]
        );
  ...
  export const getValues = async (request: any) => {
    try {
  ...
  export const upsertValue = async (payload: any) => {
    try {
      const id = toNumberOrNull(payload?.id);
      ...
      if (!resolvedDefinitionId && definitionCode) {
        const definitionResult = await query(
          `SELECT id FROM picklist_definitions WHERE code = $1`,
          [definitionCode]
        );
      ...
      const targetDefinitionResult = await query(
        `SELECT is_system_controlled FROM picklist_definitions WHERE id = $1`,
        [resolvedDefinitionId]
      );
      const targetIsSystemControlled = Boolean(
        targetDefinitionResult.rows[0]?.is_system_controlled
      );
      ...
      if (targetIsSystemControlled) {
        return {
          message:
            "New values cannot be added to a system-controlled picklist. Add workflow support in code first.",
          status: 400,
        };
      }
  ...
  export const upsertRelation = async (payload: any) => {
    try {
  ...
  export const getFieldMappings = async (request: any) => {
    try {
      const { whereClause, params } = buildQueryStringFilters(request.query || {}, {
  ...
  export const upsertFieldMapping = async (payload: any) => {
    try {
      const definitionId = toNumberOrNull(payload?.definition_id ?? payload?.definitionId);
      const definitionCode = normalizeCode(payload?.definition_code ?? payload?.definitionCode);
      let resolvedDefinitionId = definitionId;
      ...
      const result = await query(
        ...
        [
          ...
          toBoolean(
            payload?.allow_inactive_for_existing_records ??
              payload?.allowInactiveForExistingRecords,
            true
          ),
          toBoolean(payload?.is_active ?? payload?.isActive, true),
          jsonValue(payload?.metadata_json ?? payload?.metadataJson),
  ...
  export const resolvePicklists = async (request: any) => {
    try {
      const objectName = String(
        request.query?.object_name || request.query?.objectName || ""
      ).trim();
      const formName =
        request.query?.form_name || request.query?.formName
          ? String(request.query?.form_name || request.query?.formName).trim()
          : null;
      const legacyShape = toBoolean(
        request.query?.legacy_shape ?? request.query?.legacyShape
      );
      ...
      return result.rows.reduce((acc: any, row: any) => {
        if (!acc[row.definition_code]) acc[row.definition_code] = [];
  ...
  export const getBundleTemplates = async (request: any) => {
    try {
      const params: any[] = [];
      const whereClauses: string[] = [];
  ...
  export const upsertBundleTemplate = async (payload: any) => {
    try {
      const id = toNumberOrNull(payload?.id);
      const productTypeValueId = toNumberOrNull(
        payload?.product_type_value_id ?? payload?.productTypeValueId
      );
  ...
  export const upsertBundleItem = async (payload: any) => {
    try {
      const id = toNumberOrNull(payload?.id);
      const bundleTemplateId = toNumberOrNull(
        payload?.bundle_template_id ?? payload?.bundleTemplateId
      );
      const componentDefinitionId = toNumberOrNull(
        payload?.component_definition_id ?? payload?.componentDefinitionId
      );
      const componentValueId = toNumberOrNull(
        payload?.component_value_id ?? payload?.componentValueId
      );
