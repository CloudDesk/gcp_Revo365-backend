...
export interface EnquiryRecord {
    recordtype: "individual" | "corporate";
    first_name: string;
    last_name: string;
    email: string;
    phone: string;
 ...
 */
export const insertEnquiryRecord = async (
    data: EnquiryRecord
): Promise<number | null> => {
    const sql = `
  ...
  `;
    ...
    try {
        ...
        console.log(
            `[enquiryDB] ✅ Enquiry saved to DB | type: ${data.recordtype} | id: ${insertedId}`
        );
        return insertedId;
    } catch (error: any) {
        console.error(
            `[enquiryDB] ❌ Failed to save ${data.recordtype} enquiry to DB:`,
            error?.message || error
        );
 ...
 */
export const markEnquirySheetSynced = async (id: number): Promise<void> => {
    const sql = `
  ...
  `;
    ...
    try {
        ...
        console.error(
            `[enquiryDB] ❌ Failed to mark sheet_synced for id ${id}:`,
            error?.message || error
        );
