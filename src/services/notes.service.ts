...
export module notesService {
    export const getNotesData = async (request : any) => {
        try {
    ...
    export const upsertNotes = async (notesdata : any) => {
        try {
            ...
            if (id) {
                ...
                querydata = `INSERT INTO notes (${fieldNames.join(
                    ", "
                )}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
