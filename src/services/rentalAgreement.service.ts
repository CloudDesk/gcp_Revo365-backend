...
const toPositiveInteger = (value: any, fieldName = "id") => {
...
const formatAgreementNumber = (agreementId: number) => {
...
const parseDateLikeToEpoch = (value: any) => {
  try {
...
const buildPricingSnapshot = (contractRows: any[]) => {
  const items = contractRows.map((row) => ({
    ...
    totalamount:
      toNumericValue(row.productamount) * Math.max(Number(row.rentalfor ?? 1), 1),
  }));
...
const buildPenaltySnapshot = (notes: any) => ({
...
const getAgreementAssetStatus = (row: any) =>
  normalizeText(row.rentalassetstatus) ||
  (normalizeComparableText(row.rentalcontractstatus) === "active"
    ? "allocated"
    : normalizeText(row.rentalcontractstatus) || "allocated");

const getStockIdentifier = (row: any) =>
  row.stockid != null ? Number(row.stockid) : null;

const getAgreementContextRows = async (
  executor: any,
  filters: { customerId?: number | null; uniqueOrderId?: string | null; request?: any }
) => {
  ...
  const params: any[] = [ACTIVE_RENTAL_ORDER_NAME];
  ...
  await accessScopeService.appendVendorCustomerColumnScope(
    ...
    params,
    params.length + 1,
    { tableAlias: "ol", customerColumn: "userid" }
  );
...
const groupAgreementContextRows = (rows: any[]) => {
  const groupedContracts = new Map<string, any>();
...
const getAgreementList = async (
  ...
  const params: any[] = [customerId ?? null, uniqueOrderId ?? null];
  ...
  await accessScopeService.appendVendorCustomerColumnScope(
    ...
    params,
    3,
    { tableAlias: "ra", customerColumn: "customerid" }
  );
  ...
  const agreementIds = result.rows.map((row: any) => row.id);
  let assetRows: any[] = [];
  ...
  return result.rows.map((agreement: any) => ({
    ...agreement,
    assets: assetRows.filter(
      (assetRow) => Number(assetRow.agreementid) === Number(agreement.id)
    ),
  }));
};

const getAgreementDetailById = async (agreementId: number, request?: any) => {
  const agreements = await getAgreementList(null, null, request);
  const agreement = agreements.find(
    (agreementRow) => Number(agreementRow.id) === Number(agreementId)
  );
...
const syncAgreementLinks = async (
  client: any,
  agreementId: number,
...
export module rentalAgreementService {
  ...
  export const getRentalAgreements = async (request: any) => {
    const customerIdRaw = request.query?.customerid;
    const customerId =
      customerIdRaw == null || customerIdRaw === ""
        ? null
  ...
  export const createRentalAgreement = async (request: any) => {
    ...
    try {
      ...
      const agreementStatus =
        normalizeComparableText(request.body?.agreementstatus) ||
        DEFAULT_AGREEMENT_STATUS;
      const billingFrequency =
        normalizeComparableText(request.body?.billingfrequency) ||
        DEFAULT_BILLING_FREQUENCY;
      const contractRows = await getAgreementContextRows(client, {
        customerId: null,
        uniqueOrderId,
      ...
      if (
        existingAgreementResult.rows[0] &&
        NON_TERMINAL_AGREEMENT_STATUSES.has(
          normalizeComparableText(existingAgreementResult.rows[0].agreementstatus)
        )
      ) {
        throw new Error(
          `Agreement ${existingAgreementResult.rows[0].agreementnumber} already exists for this rental contract.`
        );
      }

      const groupedContract = groupAgreementContextRows(contractRows)[0];
      const primaryOrderLineId =
        requestedPrimaryOrderLineId ?? groupedContract.primaryorderlineid;
      const agreementStartDate =
        toOptionalEpoch(request.body?.agreementstartdate) ??
        groupedContract.agreementstartdate;
      const agreementEndDate =
        toOptionalEpoch(request.body?.agreementenddate) ??
        groupedContract.agreementenddate;

      if (!agreementStartDate || !agreementEndDate) {
        throw new Error(
          "Agreement start date and agreement end date are required."
        );
      }
