...
} from "./serviceEstimationStockPolicy.js";
import type { ServiceEstimationStatus } from "./serviceEstimationStockPolicy.js";
import {
    getServiceEstimationAssetAllocationPlan,
    isSingleAssetServiceEstimationUnit,
    SERVICE_ESTIMATION_CATALOGUE_STOCK_TYPES,
} from "./serviceEstimationStockEligibility.js";
import {
    getServiceEstimationTaxContext,
    resolveServiceEstimationCustomerState,
    SERVICE_ESTIMATION_GST_RATE,
} from "./serviceEstimationTaxPolicy.js";
...
const asStoredNumber = (value: any, fallback = 0) =>
    value === null || value === undefined || value === ""
        ? fallback
        : asNumber(value, fallback);

const asTaxRate = (value: any, fallback: number, fieldLabel: string) => {
...
const asPositiveInteger = (value: any, fieldLabel: string) => {
    const parsed = Number(value);
...
const parseCollection = (value: any): any[] => {
...
const isTruthyDatabaseFlag = (value: any) =>
    value === true || value === 1 || value === "1" || value === "true";

const getTicketContext = async (ticketnumber: string) => {
    ...
    return {
        ...ticketContext,
        customerstate: resolveServiceEstimationCustomerState(
            ticketContext.ticketaddressstate,
            ticketContext.latestcustomeraddressstate
        ),
    };
};
...
const resolveServiceRequestSacCode = async (_ticketContext?: any) =>
    SERVICE_COST_ESTIMATION_SAC_CODE;

const resolveProductFamilySacCode = async (_rawProductFamily?: any) =>
    SERVICE_COST_ESTIMATION_SAC_CODE;

const getExistingEstimation = async (id: any) => {
    if (!id) return null;
    const result = await query(
        "SELECT * FROM servicecostestimation WHERE id = $1 LIMIT 1",
        [id]
    );
    if (!result.rows[0]) {
...
const getProductsByIds = async (productIds: number[]) => {
    ...
    return new Map<number, any>(
        result.rows.map((product: any) => [Number(product.id), product])
    );
};

const getEligibleAssetsByIds = async (stockIds: number[]) => {
    if (stockIds.length === 0) return new Map<number, any>();
    ...
    return new Map<number, any>(
        result.rows.map((stock: any) => [Number(stock.id), stock])
    );
};

const prepareProductRows = async (rows: any[], requireSelectedProduct: boolean) => {
    ...
    const selectedAssetMap = requireSelectedProduct
        ...
        : new Map<number, any>();
    const usedAssetStockIds = new Set<number>();
    ...
    const preparedRows = activeRows.map((row, index) => {
        ...
        const hasSelectedAsset =
            Number.isInteger(incomingAssetStockId) && incomingAssetStockId > 0;
        ...
        if (requireSelectedProduct && !hasSelectedAsset) {
            throw validationError(
                `Select a valid available asset number in product row ${index + 1}`
            );
        }
        if (
        ...
        if (hasSelectedAsset && usedAssetStockIds.has(incomingAssetStockId)) {
            throw validationError(
                `The same asset number cannot be selected more than once`
            );
        }
        if (hasSelectedAsset) {
        ...
        if (
            ...
            throw validationError(
                `Product row ${index + 1} quantity must be 1 for the selected asset number`
            );
        }
        ...
        if (requireSelectedProduct && product && quantity > availableQuantity) {
            throw validationError(
                `${product.productname} has only ${availableQuantity} available; requested ${quantity}`
            );
        }

        const unitPrice = asNumber(
            row?.unitprice,
            product ? asNumber(product.price) : 0
        );
        if (unitPrice < 0) {
        ...
        if (requireSelectedProduct && !hsnCode) {
            throw validationError(
                `${product?.productname || `Product row ${index + 1}`} does not have an HSN code`
            );
        }
    ...
    if (requireSelectedProduct) {
        ...
        for (const row of preparedRows) {
            ...
            requestedByProduct.set(
                row.productid,
                (requestedByProduct.get(row.productid) || 0) + row.quantity
            );
        }
        for (const [productId, requestedQuantity] of requestedByProduct.entries()) {
            ...
            const availableQuantity = Math.max(
                0,
                Math.floor(asNumber(product?.availablequantity))
            );
            if (product && requestedQuantity > availableQuantity) {
                throw validationError(
                    `${product.productname} has only ${availableQuantity} available across all rows; requested ${requestedQuantity}`
                );
            }
        }
    }
...
const prepareServiceRows = (
    rows: any[],
    requireSacCode: boolean,
    resolvedSacCode?: string
) => {
    const activeRows = rows.filter((row) =>
        Boolean(
            String(row?.description ?? "").trim() ||
    ...
    return activeRows.map((row, index) => {
        ...
        if (requireSacCode && !/^\d{6}$/.test(sacCode)) {
            throw validationError(
                `Service row ${index + 1} SAC code must be exactly 6 numeric digits`
            );
        }
        if (unitPrice < 0) {
...
const prepareCostEstimation = async (rawInput: any) => {
    ...
    const customerState = String(
        isNew
            ...
            : merged.customerstate || ticketContext.customerstate || "Tamil Nadu"
    ).trim();
    ...
    const normalizeAddressSnapshot = (value: any) => {
        ...
        return fields.reduce((snapshot: Record<string, string>, field) => {
    ...
    if (Number.isInteger(selectedShippingAddressId) && selectedShippingAddressId > 0) {
        const addressResult = await query(
            `SELECT id, name, mobilenumber, doornumber, landmark,
                    address, city, state, pincode
             FROM address
             WHERE id = $1 AND userid = $2
             LIMIT 1`,
            [selectedShippingAddressId, ticketContext.userid]
        );
    ...
    const productRows = isNew
        ...
        : incomingProductRows;
    const hasServiceRows = incomingServiceRows.some((row) =>
        Boolean(
            String(row?.description ?? "").trim() ||
            String(row?.saccode ?? "").trim() ||
            asNumber(row?.unitprice) > 0
    ...
    const selectedProductFamily = productRows.find((row: any) => row?.productfamily)?.productfamily;
    ...
    const productSubtotal = roundMoney(
        productRows.reduce(
            (sum, row) =>
                ...
                asStoredNumber(
                    row?.totalamount,
                    asStoredNumber(row?.quantity, 1) * asStoredNumber(row?.unitprice)
                ),
            0
        )
    );
    const serviceSubtotal = roundMoney(
        serviceRows.reduce(
            (sum, row) =>
                sum + asStoredNumber(row?.totalamount, asStoredNumber(row?.unitprice)),
            0
        )
    );
    ...
    const productTotal = isNew
        ? asNonNegativeAmount(
            merged.producttotal,
            roundMoney(productSubtotal + productTaxAmount),
            "Product total"
        )
        : asStoredNumber(
            merged.producttotal,
    ...
    let estimationStatus = String(
        incoming.estimationstatus || existing?.estimationstatus || ""
    ).trim();
    if (isNew) {
        const autoApprove =
            isTruthyDatabaseFlag(ticketContext.walkintickets) ||
            (
    ...
    if (merged.estimationurl) prepared.estimationurl = merged.estimationurl;
...
const ticketTransitionForStatus = (status: string, estimationId: number) => {
...
const getRequestedProducts = (productRows: any[]) => {
    const products = new Map<
        number,
        { quantity: number; selectedStockIds: number[] }
    >();
...
const refreshCatalogueQuantities = async (
    client: any,
    affectedPucs: Set<string>
) => {
    for (const puc of affectedPucs) {
        ...
        await client.query(
            `UPDATE product_revo
             ...
             WHERE puc = $1`,
            [puc, SOLD_STOCK_STATUS]
        );
    }
};

const allocateStockForEstimation = async (
    client: any,
    estimationId: number,
    ticket: any,
    productRows: any[],
    ...
    for (const [productId, request] of requestedProducts.entries()) {
        const allocationPlan = getServiceEstimationAssetAllocationPlan(
            request.quantity,
            request.selectedStockIds
        );
        const productResult = await client.query(
            `SELECT id, puc, productname
             FROM product_revo
             WHERE id = $1
             LIMIT 1`,
            [productId]
        ...
        let selectedStockRows: any[] = [];
        if (allocationPlan.selectedStockIds.length > 0) {
            const selectedStockResult = await client.query(
                `SELECT s.id, s.puc, s.assetnumber
                 FROM stock_revo s
            ...
            if (
                ...
                throw validationError(
                    `${product.productname} selected asset is no longer available`
                );
            }
        }

        let automaticStockRows: any[] = [];
        if (allocationPlan.remainingQuantity > 0) {
            ...
            if (
                automaticStockRows.length !== allocationPlan.remainingQuantity
            ) {
                throw validationError(
                    `${product.productname} has only ${
                        selectedStockRows.length + automaticStockRows.length
                    } allocatable stock available; requested ${request.quantity}`
                );
            }
        }

        const stockIds = [...selectedStockRows, ...automaticStockRows].map(
        ...
        if (updateResult.rows.length !== request.quantity) {
            throw validationError(
                `${product.productname} stock changed while the estimation was being created`
            );
        }

        await client.query(
            `INSERT INTO service_estimation_stock_allocations (
...
const sellHeldStockForEstimation = async (
    client: any,
    estimationId: number,
    ticketId: number,
    actorId: number | null,
    ...
    const allocationResult = await client.query(
        `SELECT
         ...
         FOR UPDATE OF allocation, stock`,
        [estimationId]
    );

    if (allocationResult.rows.length === 0) return;

    const invalidStock = allocationResult.rows.find(
        (row: any) =>
            row.stockstatus !== SERVICE_HOLD_STOCK_STATUS ||
            row.holdreason !== COST_ESTIMATION_HOLD_REASON ||
            Number(row.holdticketid) !== Number(ticketId)
    );
    if (invalidStock) {
        throw validationError(
            "One or more estimation stocks are no longer on the expected Service Hold"
        );
    }

    const stockIds = allocationResult.rows.map((row: any) => Number(row.stockid));
    const allocationIds = allocationResult.rows.map((row: any) =>
        Number(row.allocationid)
    );
    ...
    if (stockUpdateResult.rows.length !== stockIds.length) {
        throw validationError(
            "Unable to mark every held estimation stock as Sold"
        );
    }

    await client.query(
        `UPDATE service_estimation_stock_allocations
         SET allocationstatus = 'sold',
             soldat = NOW(),
             modifiedby = COALESCE($1, modifiedby),
             modifiedat = NOW()
         WHERE id = ANY($2::bigint[])
           AND allocationstatus = 'held'`,
        [actorId, allocationIds]
    );

    for (const row of stockUpdateResult.rows) {
        if (row.puc) affectedPucs.add(String(row.puc));
    }
};

const restoreHeldStockForEstimation = async (
    client: any,
    estimationId: number,
    ticketId: number,
    ...
    if (stockUpdateResult.rows.length !== stockIds.length) {
        throw validationError(
            "Unable to restore every held estimation stock to Available"
        );
    }
    ...
    await client.query(
        `UPDATE service_estimation_stock_allocations
         ...
         WHERE id = ANY($3::bigint[])
           AND allocationstatus = 'held'`,
        [restorationReason, actorId, allocationIds]
    );
...
const restoreSoldStockForEstimation = async (
    client: any,
    estimationId: number,
    ticketId: number,
    restorationReason: string,
    ...
    const invalidStock = allocationResult.rows.find(
        (row: any) => row.stockstatus !== SOLD_STOCK_STATUS
    );
    if (invalidStock) {
        throw validationError(
            "One or more estimation stocks are no longer in Sold status"
        );
    }
    ...
    if (stockUpdateResult.rows.length !== stockIds.length) {
        throw validationError(
            "Unable to restore every sold estimation stock to Available"
        );
    }
...
const persistCostEstimation = async (
    id: number | null,
    prepared: Record<string, any>,
    actorId: number | null
) => {
    const filteredEntries = Object.entries(prepared).filter(
        ([field, value]) => ESTIMATION_FIELDS.has(field) && value !== undefined
    );
    const fieldNames = filteredEntries.map(([field]) => field);
    const fieldValues = filteredEntries.map(([field, value]) =>
        field === "productdata" || field === "servicedata"
    ...
    try {
        ...
        let previousEstimation: any = null;
        if (id) {
        ...
        const ticketResult = await client.query(
            `SELECT id, ticketnumber, ticketstatus, location
             FROM tickets
        ...
        const affectedPucs = new Set<string>();
        const productRows = parseCollection(saved.productdata);
        const allocationCountResult = await client.query(
            `SELECT
                COUNT(*)::int AS count,
                COUNT(*) FILTER (
             ...
             WHERE servicecostestimationid = $1`,
            [saved.id]
        );
        const allocationCount = Number(
            allocationCountResult.rows[0]?.count ?? 0
        );
        const soldAllocationCount = Number(
            allocationCountResult.rows[0]?.soldcount ?? 0
        );
        const restoredAllocationCount = Number(
            allocationCountResult.rows[0]?.restoredcount ?? 0
        );

        const stockPlan = getServiceEstimationStockPersistencePlan({
            isNew: !id,
            previousStatus:
                (previousEstimation?.estimationstatus as ServiceEstimationStatus) ??
                null,
            nextStatus: saved.estimationstatus as ServiceEstimationStatus,
            allocations: {
        ...
        if (stockPlan.allocate) {
            await allocateStockForEstimation(
                client,
                Number(saved.id),
                ticket,
                productRows,
        ...
        if (stockPlan.sellHeld) {
            await sellHeldStockForEstimation(
                client,
                Number(saved.id),
                Number(ticket.id),
                actorId,
            ...
            await restoreHeldStockForEstimation(
                client,
                Number(saved.id),
                Number(ticket.id),
                stockPlan.heldRestorationReason as string,
                actorId,
                affectedPucs
            );
        }

        const transition = ticketTransitionForStatus(
            saved.estimationstatus,
            Number(saved.id)
        );
        if ("approvedcostestimationid" in transition) {
            ...
            await client.query(
                "UPDATE tickets SET ticketstatus = $1 WHERE ticketnumber = $2",
                [transition.ticketstatus, saved.ticketnumber]
            );
        }

        await refreshCatalogueQuantities(client, affectedPucs);
        await client.query("COMMIT");
        (result as any).affectedPucs = Array.from(affectedPucs);
        return result;
    } catch (error) {
    ...
    try {
        ...
        if (estimationstatus === "waiting_for_approval") {
            ...
            text = templates.waiting_for_cost_estimation_approval.text
                ...
                .replace(
                    "{totalPayable}",
                    totalpayableamount != null ? `₹${totalpayableamount}` : "N/A"
                )
            ...
            text = templates.unresolved_closed.text.replace(
                "{ticketNumber}",
                ticketnumber
            );
        } else {
        ...
        console.error(
            `[costEstimation] Email failed for estimationstatus "${estimationstatus}":`,
...
const saveAndNotify = async (
    ...
    await fireEstimationEmail(
        saved.estimationstatus,
        saved.ticketnumber,
        saved.estimationurl ?? null,
        saved.totalpayableamount ?? null
    );
    return result;
};

export module costEstimationService {
    export const getEstimationProducts = async (request: any) => {
        ...
        let serviceSacCode: string | null = null;
        let serviceSacResolutionError: string | null = null;
        if (ticketnumber) {
            const ticketContext = await getTicketContext(ticketnumber);
            ...
            billingAddressId = customerAddresses.some(
                (address) => Number(address.id) === Number(ticketContext.addressid)
            )
                ? Number(ticketContext.addressid)
                : Number(customerAddresses[0]?.id) || null;
            defaultShippingAddressId = customerAddresses.some(
                (address) => Number(address.id) === Number(ticketContext.addressid)
            )
                ? Number(ticketContext.addressid)
                : Number(customerAddresses[0]?.id) || null;
    ...
    export const getEstimationProductAssets = async (request: any) => {
    ...
    export const getCostEstimationData = async (request: any) => {
        ...
        for (const [key, rawValue] of Object.entries(request.query)) {
            ...
            if (key === "page" || key === "count") continue;
    ...
    export const getStockRestorationStatus = async (request: any) => {
        ...
        const result = await query(
            `SELECT
             ...
             LIMIT 1`,
            [estimationId, SOLD_STOCK_STATUS]
        );

        const record = result.rows[0];
        if (!record) {
            throw validationError(
                `Cost estimation ${estimationId} was not found`
            );
        }
    ...
    export const restoreApprovedEstimationStock = async (request: any) => {
        const estimationId = Number(request.params?.id);
        if (!Number.isInteger(estimationId) || estimationId <= 0) {
            throw validationError("A valid cost estimation id is required");
        }
        ...
        try {
            ...
            const estimationResult = await client.query(
                `SELECT
                 ...
                 JOIN tickets ticket
                   ON ticket.ticketnumber = estimation.ticketnumber
                 WHERE estimation.id = $1
                 FOR UPDATE OF estimation, ticket`,
                [estimationId]
            );
            ...
            if (
                estimation.estimationstatus !== "approved" ||
                estimation.ticketstatus !== "service_in_progress"
            ) {
                throw validationError(
                    "Stock restoration is allowed only for an approved service in progress"
                );
            }
            ...
            if (invoiceResult.rows.length > 0) {
                throw validationError(
                    "Stock cannot be restored after the final invoice has been generated"
                );
            }

            const affectedPucs = new Set<string>();
            const restoredQuantity = await restoreSoldStockForEstimation(
                client,
                estimationId,
                Number(estimation.ticketid),
                restorationReason,
                actorId,
                affectedPucs
            );
            if (restoredQuantity === 0) {
                throw validationError(
                    "No sold estimation stock is available to restore"
                );
            }
    ...
    export const upsertCostEstimation = async (request: any, input: any) => {
        ...
        if (!preparedResult.id) {
            const documentResult: any = await GenerateDocx(
                request,
                [preparedResult.documentData],
                "costestimation/costestimation.docx"
            );
            if (!documentResult?.fileurl) {
                throw validationError(
                    typeof documentResult === "string"
                        ? documentResult
    ...
    export const upsertGcpCostEstimation = async (request: any, input: any) => {
        const preparedResult = await prepareCostEstimation(input);
        const sessionActorId = Number(request.session?.id);
        const actorId = Number.isInteger(sessionActorId)
            ? sessionActorId
