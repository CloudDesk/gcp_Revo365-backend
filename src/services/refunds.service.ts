...
export module refundsService {
  const ACTIVE_REFUND_STATUSES = ["initiated", "pending", "processed", "manual_done"];

  const resolveSessionUserId = (request: any): number | null => {
    const raw =
      request?.session?.id ??
      request?.user?.id ??
      request?.body?.userid ??
  ...
  const toPaise = (value: any): number | null => {
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed <= 0) return null;
    return Math.round(parsed * 100);
  };

  const fromPaise = (value: any): number => {
    const parsed = Number(value);
  ...
  const normalizeGatewayRefundStatus = (value: any): string => {
  ...
  const resolveTargetOrderLine = async (input: any) => {
    if (input?.orderlineid) {
      const result = await query(
        `SELECT * FROM orderline WHERE id = $1 LIMIT 1`,
        [Number(input.orderlineid)]
      );
    ...
    if (input?.orderlinenumber) {
      const result = await query(
        `SELECT * FROM orderline WHERE orderlinenumber = $1 LIMIT 1`,
        [String(input.orderlinenumber)]
      );
      return result.rows[0] || null;
    }
    ...
    if (input?.merchanttransactionid && input?.productid) {
      const result = await query(
        `SELECT *
         ...
         ORDER BY id DESC
         LIMIT 1`,
        [String(input.merchanttransactionid), Number(input.productid)]
      );
      return result.rows[0] || null;
    }
  ...
  const getLatestReturnRequest = async (orderlineId: number) => {
    const result = await query(
      `SELECT *
  ...
  const getLatestPaymentContext = async (merchanttransactionid: string) => {
    const result = await query(
      `SELECT transactionid, razorpay_payment_id, razorpay_order_id, transactiondata
       FROM transaction
  ...
  const getRefundSummary = async (orderlineId: number, razorpayPaymentId: string) => {
    const [lineSummary, paymentSummary] = await Promise.all([
      query(
        ...
        [orderlineId, ACTIVE_REFUND_STATUSES]
      ),
      query(
        `SELECT COALESCE(SUM(amount_paise), 0)::bigint AS total_amount_paise
         FROM payment_refunds
         WHERE razorpay_payment_id = $1
           AND status = ANY($2::text[])`,
        [razorpayPaymentId, ACTIVE_REFUND_STATUSES]
      ),
    ]);
  ...
  const updateLinkedReturnRefundState = async (
    orderlineId: number,
    status: string,
    refundReference: string | null
  ) => {
    await query(
      `UPDATE orderline_returns
       SET refund_status = $1,
  ...
  const getRefundEligibilityContext = async (input: any) => {
    ...
    const summary = await getRefundSummary(
      Number(lineRow.id),
      String(paymentContext.razorpay_payment_id)
    );
  ...
  export const getRefunds = async (request: any) => {
  ...
  export const getRefundEligibility = async (request: any) => {
  ...
  export const initiateRefund = async (request: any) => {
    ...
    const requestedAmountPaise =
      toPaise(body?.amount) ?? maxRefundablePaise;
    ...
    try {
      ...
      if (latestReturn?.id) {
        await updateLinkedReturnRefundState(
          Number(lineRow.id),
          normalizedStatus,
          refund?.id || null
        );
  ...
  export const syncRefund = async (request: any) => {
    const body = request?.body || {};
    let refundRow: any = null;

    if (body?.id) {
      const result = await query(
        `SELECT * FROM payment_refunds WHERE id = $1 LIMIT 1`,
        [Number(body.id)]
      );
      refundRow = result.rows[0] || null;
    } else if (body?.razorpayRefundId) {
      const result = await query(
        `SELECT * FROM payment_refunds WHERE razorpay_refund_id = $1 ORDER BY created_at DESC, id DESC LIMIT 1`,
        [String(body.razorpayRefundId)]
      );
      refundRow = result.rows[0] || null;
    } else if (body?.orderlineid) {
      const result = await query(
        `SELECT * FROM payment_refunds WHERE orderlineid = $1 ORDER BY created_at DESC, id DESC LIMIT 1`,
    ...
    try {
      const refund = await razorpay.payments.fetchRefund(
        String(refundRow.razorpay_payment_id),
        String(refundRow.razorpay_refund_id)
      );
      const normalizedStatus = normalizeGatewayRefundStatus(refund?.status);
      ...
      if (refundRow?.return_request_id) {
        await updateLinkedReturnRefundState(
          Number(refundRow.orderlineid),
          normalizedStatus,
          refund?.id || refundRow.razorpay_refund_id
        );
      }
      ...
      await query(
        `UPDATE payment_refunds
         SET gateway_error = $1,
             ...
             updated_at = NOW()
         WHERE id = $2`,
        [safeJson(error?.error || error?.response?.data || error?.message || error), Number(refundRow.id)]
      );
